...
Cook proved in 2004 that Rule~110 can simulate any Turing machine.  The
result has been independently formalized in Lean~4 via an algebraic
universality route that bypasses Cook's original construction:
\begin{itemize}
  \item \lean{rule110\_turing\_universal\_algebraic} (\CatAL, zero sorry,
    rule110-lean): Rule~110 is Turing-universal by the NAND algebraic route.
\end{itemize}
...
a pole is geometrically different from one that winds once.

The three non-vacuum orbit classes that the GTE polynomial produces over
$\mathrm{GF}(7)$ correspond to the three generations of charged fermions.
The orbit structure responsible for this is the \term{color group}
$\mathbb{Z}_3$ — a cyclic group of three elements.
...
produces three distinct non-vacuum orbit classes.  Each class is
characterized by its topological winding number modulo 7:

\begin{itemize}
  \item \textbf{Orbit class A} (winding $+2$ mod 7): maps to the
    first-generation sector.  This is the lightest generation.
    It is a \term{Garden of Eden} (GoE) configuration — it has no
    predecessor under the rule, which makes it arithmetically ``most
    stable'' and corresponds to why the electron is the lightest fermion.
  \item \textbf{Orbit class B} (winding $+3$ mod 7): second-generation sector.
  \item \textbf{Orbit class C} (winding $+4$ mod 7): third-generation sector.
\end{itemize}

The three winding numbers $\{2, 3, 4\}$ are exactly the three non-unit
elements of the Sylow-3 subgroup $\{1, 2, 4\}$ and the adjacent elements
of $\mathrm{GF}(7)^*$.  This is not a coincidence; it is a consequence of
the polynomial's arithmetic over the field.
...
But orbit class~B has winding charge $Q = 2$.  Under the $\mathbb{Z}_2$
projection ($w \mapsto w \bmod 2$), $2$ maps to $0$ — the same as the vacuum.
So a second-generation particle would be misidentified as the vacuum.
...
\lean{mdl\_z7z3\_beats\_z7z2} (\CatAL, ugp-lean) certifies this comparison.
...
\lean{rule110\_turing\_universal\_algebraic}
  & Rule~110 is Turing-universal (algebraic route) \\
...
All theorems: Lean~4, zero sorry, zero custom axioms, ugp-lean repository.
