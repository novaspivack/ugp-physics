...
a separate ingredient — it is a stable \emph{twist} in the $\Phi_{\rm MDL}$ field
itself, where the field transitions from one vacuum state to another without
being able to unwind.  This twist is not a compact spatial blob: a general
theorem proves that any field configuration carrying a nonzero twist in three
spatial dimensions is necessarily \emph{extended} (wall-like), never a localized
lump of finite size.  What identifies a particle is not that it occupies a small
region of space, but that its mass, charge, spin, and generation are realized as
a normalizable excitation within its topological sector, exactly as the Standard
Model's particles are excitations of quantum fields rather than localized objects.
\end{tcolorbox}
...
Particles are not added separately — they emerge as excitations of this field,
certified by classical field configurations carrying a conserved topological
invariant (winding number) called \term{topological kink solitons}
...
\item \textbf{A particle is a normalizable excitation certified by a topological
  kink} of the $\Phi_{\rm MDL}$ field — a stable transition between two
  $\mathbb{Z}_7$ vacuum sectors that is necessarily extended in three spatial
  dimensions, never a compact lump.  It cannot decay without violating topology.
