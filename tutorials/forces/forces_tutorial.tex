...
\textbf{Why $c_H = 13$?}
The number 13 is not arbitrary: it is forced by the electroweak boson
``staircase.'' The three electroweak bosons occupy consecutive GTE branch
capacities with unit steps between them: $c_{W^+} = 11$, $c_Z = 12$,
$c_{H^0} = 13$. Equivalently, $c_H = \Ngen + 2N_{\rm fam} = 3 + 10 = 13$,
where $N_{\rm fam} = 5$ is the SM family count (the $\mathbb{Z}_5$ ring size).
Lean cert: \lean{ew\_c\_staircase} (\CatAL).
The derivation is in P31~\cite{SpivackWeinbergAngle} and P48~\cite{SpivackGTECompleteFramework}.
