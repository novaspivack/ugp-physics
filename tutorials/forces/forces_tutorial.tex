...
\midrule
0 & Vacuum, photon, $Z^0$, neutrinos & Neutral / massless gauge sector \\
1 & Lepton (gen-1), photon & Electromagnetic (QED) \\
2 & Lepton (gen-2) & Muonic charged current \\
3 & Lepton (gen-3), $W^\pm$ & Weak charged current \\
4 & Quark (down-type), $\bar{q}$ & Charged weak, CKM mixing \\
5 & Quark (up-type) & Up-quark current \\
6 & $W^+$ angular mode & $W^+$ flavor-change \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{align}
  ...
  M_Z &= M_W \cdot \sqrt{\frac{\cH}{\cH - \Ngen}}
       ...
       = 91.6~\text{GeV} \quad (\CatAL;\;\lean{m\_Z\_gte\_from\_wolfenberg}).
  \label{eq:mz}
\end{align}
...
\end{itemize}
Lean cert: \lean{gte\_charge\_formula\_from\_centering} (\CatAL, zero sorry)~\cite{SpivackGTECompleteFramework}.
