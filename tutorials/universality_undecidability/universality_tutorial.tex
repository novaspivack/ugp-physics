...
\end{itemize}
Two independent proofs from two independent mathematical directions both certify that
the substrate's binary sector \emph{is} Rule~110 --- and Rule~110 itself is
Turing-universal by Matthew Cook's 1994/2004 construction~\cite{Cook2004}.

A separate, purely algebraic fact about Rule~110 is also true and worth knowing,
but it does \emph{not} add a third, independent route to universality: at the
center cell, $p(L,1,R) = 1 - LR = \mathrm{NAND}(L,R) \pmod 7$, and NAND is
functionally complete for finite Boolean logic (Sheffer 1913).  Functional
completeness over a fixed, finite number of inputs is a genuinely different ---
and strictly weaker --- statement than Turing-universality, which requires
simulating arbitrary computations on an unbounded tape for an unbounded number
of steps; no finite-input identity can establish that by itself.  This
finite-completeness fact is machine-certified as
\lean{rule110\_center1\_nand\_functional\_completeness} (CatAL, zero sorry) ---
a genuinely correct and useful result, just not a Turing-universality proof.
\end{tcolorbox}
...
\item The GTE substrate achieves universality from two independent directions
  (UWCA, orbit parity shadow), both machine-certified and both resting on
  Cook's 1994/2004 proof that Rule~110 itself is Turing-universal.  A third,
  separate machine-certified fact --- that Rule~110's center cell realizes
  NAND, and NAND is finitely functionally complete --- is true and useful but
