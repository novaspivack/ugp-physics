...
and Gauge Couplings from GTE}\\[6pt]
\large How two related $\mathbb{Z}_7$ cellular automaton rules encode the strengths\\
...
cannot explain their values.  The GTE framework derives them from two
MDL-minimal $\mathbb{Z}_7$ cellular automaton rules --- a 19-bit polynomial
rule $p$ for the electromagnetic-sector identity of \S2--\S3, and a
separate, algebraically distinct 76-bit interaction rule $f_{\rm MDL}$ for the
weak-mixing-angle derivation of \S5 --- with zero free parameters in either
case.
\end{tcolorbox}
...
\begin{tcolorbox}[colback=black!5,colframe=black!50,
  title=\textbf{A note on which rule is used here}]
This section uses a rule called $f_{\rm MDL}$, \emph{not} the polynomial $p$
used above for the fine-structure-constant identity ($\alpha^{-1}=2^7+3^2=137$).
The
two are proved to be algebraically and dynamically distinct: $f_{\rm MDL}$ is
the unique minimum-description-length map (76 bits to specify, as against
19 bits for $p$) that agrees with Wolfram's binary Rule~110 on purely binary
inputs \emph{and} reproduces the observed three-generation Standard Model
orbit; a companion paper's Schwartz--Zippel zero-counting argument shows
$f_{\rm MDL}$ is \emph{not} a low-degree polynomial at all (it has only 14
nonzero outputs among 343 possible inputs, whereas $p$ has 300). The two rules
agree only on the 8 purely binary inputs they share as a common Rule~110
sublayer. Every neighborhood count below is a count of $f_{\rm MDL}$'s
outputs.
\end{tcolorbox}

This is the heart of the derivation.  The ratio $3/13$ is not guessed; it
emerges from a count of specific structural features of the $f_{\rm MDL}$ rule:
...
\begin{tcolorbox}[colback=boxred,colframe=red!50!black,title=Certification status --- what this derivation does and does not claim]
\begin{itemize}[itemsep=3pt]
  \item \textbf{Established ($\mathbf{A}_{\rm Lean}$/A):} the four structural
    inputs $(\Ngen,\Nfam,b_H,\cH)=(3,5,3,13)$; the exact counts of active and
    palindromic $f_{\rm MDL}$ neighborhoods (machine-certified,
    \lean{native\_decide}, zero sorry); the arithmetic identity
    $\sinwsq=b_H/\cH=3/13$; the independent GUT-scale cross-check $3/8$.
  \item \textbf{Conditional on one named import (A/D, conditional):} the
    specific physical labeling of \emph{which} channel is $U(1)_Y$ and which
    is $SU(2)_L$ uses one imported theorem from a companion dynamics paper
    (\lean{doublet\_partner\_is\_left\_chiral}).  Without it, the arithmetic
    split $3+10=13$ still holds, but the theory would not yet say which
    physical force gets which count.
  \item \textbf{Not claimed:} this section does not re-derive why $f_{\rm MDL}$
    itself is the correct interaction rule (that separate MDL-minimality
    argument is in the primary sources cited in Further Reading); it does not
    walk through the full two-loop running calculation in detail.
\end{itemize}
\end{tcolorbox}
...
enough algebraic structure to determine the electromagnetic coupling strength.
The GTE symmetry group $\ZS$ has order 7, giving $2^7 = 128$ electromagnetic
configurations.  Three generations give $3^2 = 9$ lepton-loop weight.  Their
sum is $\alpha^{-1} = 137$.  The weak mixing angle uses a related but
algebraically distinct $\mathbb{Z}_7$ rule, $f_{\rm MDL}$ (the two rules are
proved to share only their common Rule~110 binary sublayer; $f_{\rm MDL}$
itself is provably not a low-degree polynomial, unlike $p$): $f_{\rm MDL}$ has
13 active neighborhoods, of which 3 are palindromes; their ratio is
$\sinwsq = 3/13$.  Both results are Lean-certified with zero sorry.  Neither
number was adjusted to fit experiment.
\end{tcolorbox}
...
\begin{itemize}
  \item Weinberg angle derivation (P31~\cite{SpivackWeinberg}): \url{https://doi.org/10.5281/zenodo.20319413}
  \item GTE complete theory monograph (P48): \url{https://doi.org/10.5281/zenodo.20560550}
  \item Uniqueness of the Universal Generative Principle (P05): \url{https://doi.org/10.5281/zenodo.20169932}
