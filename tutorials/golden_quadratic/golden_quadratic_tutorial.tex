...
\begin{tcolorbox}[colback=green!4,colframe=green!55!black,
  title={\textbf{Two Levels: Certificate and Field}},
  fonttitle=\small\bfseries, left=6pt, right=6pt, top=4pt, bottom=4pt]
\small
GTE is a \emph{two-level} theory.  The polynomial
$p(L,C,R) = C+R-CR-LCR$ over $\mathrm{GF}(7)$ is the Level~1
\emph{algebraic certificate}: it certifies which structures physics must contain.
The Level~2 \emph{physical substrate} is the $\Phi_{\mathrm{MDL}}$ field,
$\mathcal{L} = \tfrac{1}{2}(\partial_\mu\Phi)^2 - \tfrac{m^2}{49}(1-\cos 7\Phi)
+ \mathcal{L}_{\rm gauge} + \mathcal{L}_\chi$;
elementary particles are BPS kink solitons of this field, not cells of a CA.
The algebraic results in this tutorial (golden root, binary floor) are
Level~1 certificates; full derivation of the physics runs through the Level~2
field.  Full treatment: P48 \S\,Theory at a Glance.
\end{tcolorbox}
