...
The strong CP resolution is one result among many that emerge from the GTE framework.  Without fitting any free parameters to experimental data, the framework derives:
...
Strong CP and several of the listed quantities follow from the orbit arithmetic of the MDL-minimal 19-bit rule~$p$; $\sin^2\theta_W = 3/13$ is a separate closed result from the active-neighborhood count of the related rule~$f_{\mathrm{MDL}}$, not from the coefficients of~$p$ alone.  All of these from zero free parameters --- structural consequences of the framework, not empirical fitting.
