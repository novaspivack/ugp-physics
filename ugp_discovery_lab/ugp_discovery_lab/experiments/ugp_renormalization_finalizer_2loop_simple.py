# ugp_discovery_lab/experiments/ugp_renormalization_finalizer_2loop_simple.py
"""
UGP Renormalization Finalizer 2-Loop Simple Experiment

This experiment implements a simplified 2-loop correction by adding a small
2-loop term to the 1-loop U(1) beta function. This approach is more conservative
and should provide a modest improvement over the 1-loop result without the
complexity of full 2-loop cross-coupling effects.

The 2-loop correction is: β₁ = (1/16π²) * b₁₁ * g₁³ + (1/16π²)² * b₁₂ * g₁⁵
where b₁₂ is the 2-loop self-coupling coefficient.
"""
...
def get_b1_scale_dependent_2loop(mu: float, particle_catalog: pd.DataFrame, hypercharge_model: Dict[str, Any]) -> Tuple[float, float]:
    """
    Calculate the 1-loop and 2-loop beta function coefficients for U(1).
    
    Returns (b1_1loop, b1_2loop) where:
    - b1_1loop is the 1-loop coefficient (41/6)
    - b1_2loop is the 2-loop self-coupling coefficient (199/18)
    """
    ...
    # Use exact Standard Model coefficients
    b1_1loop = 41.0 / 6.0   # U(1) 1-loop coefficient
    b1_2loop = 199.0 / 18.0 # U(1) 2-loop self-coupling coefficient
...
def rge_2loop_simple_rhs(ln_mu: float, alpha: float, particle_catalog: pd.DataFrame, hypercharge_model: Dict[str, Any]) -> float:
    """
    Right-hand side of the 2-loop RGE for α(μ) with simple 2-loop correction.
    
    dα/d(ln μ) = β₁(μ) = (1/16π²) * b₁₁ * α² + (1/16π²)² * b₁₂ * α³
    
    where b₁₁ is the 1-loop coefficient and b₁₂ is the 2-loop coefficient.
    """
    mu = math.exp(ln_mu)
    b1_1loop, b1_2loop = get_b1_scale_dependent_2loop(mu, particle_catalog, hypercharge_model)
...
@register_experiment("ugp_renormalization_finalizer_2loop_simple")
class UGPRenormalizationFinalizer2LoopSimple(Experiment):
    """
    Simplified 2-loop RGE experiment that adds a conservative 2-loop correction
    to the 1-loop U(1) beta function to improve precision.
    """
    
    def tasks(self) -> List[Dict[str, Any]]:
        return [{"task_id": "ugp_renormalization_2loop_simple"}]

    def run_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Run the simplified 2-loop UGP renormalization finalizer experiment."""
        logger.info(f"Starting Simplified 2-Loop UGP Renormalization Finalizer: {task['task_id']}")
        ...
        # Convert initial condition to alpha
        alpha_initial = bare_g1_squared / (4.0 * math.pi)
        
        logger.info(f"Initial α = {alpha_initial:.10f}")
        ...
        logger.info("Integrating simplified 2-loop RGE with full GTE spectrum...")
        try:
            sol = solve_ivp(
                rge_wrapper,
            ...
            logger.info(f"Final α = {alpha_final:.10f}")
            ...
            # Ensure particle_catalog is a DataFrame before plotting
            if isinstance(particle_catalog, pd.Series):
                particle_catalog = particle_catalog.to_frame().T
            elif not isinstance(particle_catalog, pd.DataFrame):
                logger.error("particle_catalog is not a DataFrame or Series; skipping plot generation.")
            else:
    ...
    def _generate_plots(self, sol, particle_catalog: pd.DataFrame, hypercharge_model: Dict[str, Any], 
        ...
        """Generate plots showing the simplified 2-loop RG running."""
        try:
            # Create output directory
            plots_dir = self.root / "plots"
            plots_dir.mkdir(exist_ok=True)
            
            # Plot 1: Simplified 2-loop RG running
            ...
            # Plot g₁² vs log(μ)
            ax1.semilogx(mu_points, g1_squared_points, 'b-', linewidth=2, label='g₁²(μ) 2-loop simple')
            ax1.axhline(y=0.1279, color='r', linestyle='--', alpha=0.7, label='Experimental g₁²(M_Z) = 0.1279')
            ax1.set_xlabel('Scale μ (GeV)')
            ax1.set_ylabel('g₁²(μ)')
            ax1.set_title('Simplified 2-Loop RG Running of U(1) Gauge Coupling')
            ax1.legend()
            ax1.grid(True, alpha=0.3)
            
            # Plot α vs log(μ)
            ax2.semilogx(mu_points, alpha_points, 'g-', linewidth=2, label='α(μ) 2-loop simple')
            ...
            plt.tight_layout()
            plt.savefig(plots_dir / "2loop_simple_rg_running.png", dpi=300, bbox_inches='tight')
            plt.close()
            
            logger.info(f"Simplified 2-loop plots saved to {plots_dir}")
            
        except Exception as e:
            logger.warning(f"Failed to generate simplified 2-loop plots: {e}")

    def summarize(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Summarize the simplified 2-loop UGP renormalization finalizer results."""
        ...
        if not successful_results:
            summary = {
                ...
                "success_rate": 0.0,
                "status": "failed",
                "message": "No successful simplified 2-loop RG integrations"
            }
        else:
        ...
        # Write reports
        write_json_report(self.root, "ugp_renormalization_finalizer_2loop_simple_summary", summary)
        
        md_content = [
            "# UGP Renormalization Finalizer 2-Loop Simple — Summary",
            "",
            ...
            "## Interpretation",
            "",
            "This simplified 2-loop calculation includes:",
            "- **1-loop U(1) beta function** with full GTE particle spectrum",
            "- **2-loop self-coupling correction** (b₁₂ = 199/18)",
            "- **Conservative approach** avoiding complex cross-coupling effects",
            "",
            "The 2-loop correction should provide a modest improvement",
            "over the 1-loop result, bringing us closer to the experimental value."
        ]
        
        write_md_report(self.root, "ugp_renormalization_finalizer_2loop_simple_summary", "\n".join(md_content))
