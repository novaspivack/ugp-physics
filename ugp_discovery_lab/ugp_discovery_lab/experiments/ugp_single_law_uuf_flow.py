...
class UGPSingleLawUUFFlow(Experiment):
    ...
    def __init__(self, config: Dict[str, Any], root: Path):
        ...
        # BREAKTHROUGH: Using enhanced neutrino triples that achieved 24.95% PMNS error
        nuR_breakthrough = [(1, 4, 9), (16, 25, 36), (49, 64, 81)]  # Squares - 7.85% improvement!
        nuR_default = [(2, 5, 5), (7, 11, 13), (17, 19, 23)]
        self.nu_R_triples_cfg = tuple(
            tuple(triple) for triple in 
            config.get('options', {}).get('nu_R_triples', nuR_breakthrough)
        )
