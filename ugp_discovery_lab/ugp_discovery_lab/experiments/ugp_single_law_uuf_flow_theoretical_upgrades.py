...
Cross-Reference: Physics_Papers/01_A_First-Principles_Derivation_Standard Model/first_principles_standard_model.tex
...
RECENT UPDATE (2025-01-27): Enhanced with Z₆ hexagonal symmetry CP phase extraction
...
class UGPSingleLawUUFFlowTheoreticalUpgrades(Experiment):
    ...
    def _calculate_jarlskog_invariant(self, U: np.ndarray) -> float:
        """
        Calculate Jarlskog invariant from mixing matrix.
        
        J = Im(U_11 U_22 U_12* U_21*)
        
        This is basis-invariant and measures CP violation strength.
        """
        return float(np.imag(U[0,0] * U[1,1] * np.conj(U[0,1]) * np.conj(U[1,0])))
    
    def _extract_cp_phase_z6(self, J: float, angles: Dict[str, float], 
                             mixing_matrix: Optional[np.ndarray] = None) -> Dict[str, Any]:
        """
        Extract CP phase from Jarlskog invariant using Z₆ hexagonal symmetry.
        
        Uses gauge group center structure (Baez 2003, Bakker et al. 2004):
        - Z₆ values: 0°, 60°, 120°, 180°, 240°, 300° (k·π/3 for k=0,1,2,3,4,5)
        
        ENHANCED: Now uses full complex phase information to handle δ ∈ [0°, 360°)
        
        Args:
            J: Jarlskog invariant
            angles: Dict with theta12, theta13, theta23 in degrees
            mixing_matrix: Optional complex unitary matrix for full phase extraction
        
        Returns:
            Dict with raw delta_cp_deg, z6_delta_cp_deg, z6_k_value, jarlskog, z6_angle_name
        """
        d2r = np.pi / 180.0
        
        # Method 1: Try to get full phase from matrix if available
        delta_full_deg = None
        if mixing_matrix is not None:
            # Extract full phase from U_e3 = s13 exp(-i·delta)
            # delta = -arg(U_e3)
            U_e3 = mixing_matrix[0, 2]
            if abs(U_e3) > 1e-12:
                delta_from_matrix = -np.angle(U_e3)
                delta_full_deg = np.degrees(delta_from_matrix)
                # Normalize to [0, 360)
                delta_full_deg = delta_full_deg % 360
        
        # Method 2: Jarlskog gives sin(delta), infer full phase
        if delta_full_deg is None:
            t12, t13, t23 = [angles[k] * d2r for k in ('theta12', 'theta13', 'theta23')]
            ...
            # arcsin only gives [-90°, 90°], but we can infer other quadrants
            # by testing all Z₆ values and seeing which matches sin(delta) best
            delta_candidates = []
            z6_values_deg = [0, 60, 120, 180, 240, 300]
            
            for z6_deg in z6_values_deg:
                z6_rad = z6_deg * d2r
                sin_z6 = np.sin(z6_rad)
                error = abs(sin_z6 - sin_delta)
                delta_candidates.append((error, z6_deg))
            
            # Also try negative versions
            for z6_deg in z6_values_deg:
                z6_rad = -z6_deg * d2r
                sin_z6 = np.sin(z6_rad)
                error = abs(sin_z6 - sin_delta)
            ...
            delta_full_deg = delta_candidates[0][1] % 360
        
        # Find nearest Z₆ value to full phase
        ...
        return {
            'jarlskog': J,
            'delta_cp_raw_deg': float(delta_full_deg),
            'delta_cp_z6_deg': float(best_z6_deg),
            'delta_cp_z6_normalized_deg': float(best_z6_deg),
            'z6_k_value': k_value,
            'z6_angle_name': f'k={k_value}, π·{k_value}/3 = {best_z6_deg:.0f}°'
        }
    
    def _extract_mixing_angles(self, mixing_matrix: np.ndarray) -> Dict[str, float]:
        """
        Extract mixing angles and CP phase from unitary matrix.
        
        UPDATED (2025-01-27): Now includes Z₆ hexagonal symmetry CP phase extraction.
        
        Returns:
            Dict with theta12, theta13, theta23 (degrees) and CP phase information
        """
        # Extract angles (exact match with working config)
        ...
        angles = {"theta12": t12, "theta13": t13, "theta23": t23}
        
        # Calculate Jarlskog invariant and CP phase
        J = self._calculate_jarlskog_invariant(mixing_matrix)
        cp_info = self._extract_cp_phase_z6(J, angles, mixing_matrix)
        
        # Combine results
        result = {**angles, **cp_info}
        
        return result
    
    def _calculate_ckm_errors(self, ckm_angles: Dict[str, float]) -> Dict[str, float]:
        """
        Calculate CKM angle and CP phase errors.
        
        UPDATED (2025-01-27): Now includes Z₆ CP phase error calculation.
        """
        ...
        # Add CP phase error if available
        if 'delta_cp_z6_normalized_deg' in ckm_angles:
            # Experimental CKM delta_cp ≈ 68.8° (PDG)
            exp_delta_cp = 68.8
            pred_delta_cp = ckm_angles['delta_cp_z6_normalized_deg']
            
            # Circular error (handle wraparound)
            error_deg = abs(pred_delta_cp - exp_delta_cp)
            if error_deg > 180:
                error_deg = 360 - error_deg
            
            errors["delta_cp_error"] = error_deg / exp_delta_cp
            errors["delta_cp_error_deg"] = error_deg
            errors["delta_cp_pred_deg"] = pred_delta_cp
            errors["delta_cp_exp_deg"] = exp_delta_cp
        
        return errors
    
    def _calculate_pmns_errors(self, pmns_angles: Dict[str, float]) -> Dict[str, float]:
        """
        Calculate PMNS angle and CP phase errors.
        ...
        # Add CP phase error if available
        if 'delta_cp_z6_normalized_deg' in pmns_angles:
            # Experimental PMNS delta_cp ≈ 195° (T2K/NOvA hint)
            exp_delta_cp = 195.0
            pred_delta_cp = pmns_angles['delta_cp_z6_normalized_deg']
