# UGP-Locked Flow Normalized Theoretical Implementation
# ======================================================================================================
# This implements the refined flow-based approach with:
# - Theory-consistent time (nats from bits)
# - Generator-normalized flows (spectral radius normalization)
# - Exact closed-form flow (no ODE discretization)
# - Unit-consistent scaling for proper hierarchy
...
@dataclass
class FlowNormalizedResult:
    """Results from normalized flow theoretical implementation."""
    ckm_matrix: np.ndarray
    pmns_matrix: np.ndarray
    ckm_angles: Dict[str, float]
    pmns_angles: Dict[str, float]
    ckm_score: float
    perm_choice: Tuple[int, int, int]
    neutrino_model: str
    flow_parameters: Dict[str, float]
    generator_normalization: Dict[str, Any]
...
@register_experiment("ugp_yukawa_ckm_pmns_flow_normalized")
class UGPYukawaCKMPMNSFlowNormalized(Experiment):
    """
    Normalized flow theoretical implementation for CKM/PMNS mixing matrices.
    ...
    def __init__(self, config: Dict[str, Any], root: str):
        ...
        # Theory-consistent time: convert bits to nats
        self.L_residual = config.get("residual_kraft_length", 9.382)
        self.tau0 = math.log(2) * self.L_residual  # nats from residual Kraft length
    ...
    def _spectral_radius(self, matrix: np.ndarray) -> float:
        """Compute spectral radius (2-norm) of matrix."""
        return np.linalg.norm(matrix, ord=2)
    
    def _build_generators(self, triples_list: List[Tuple[int, int, int]], gens: List[int], sector: str) -> Tuple[np.ndarray, np.ndarray, float, float]:
        """
        Build normalized generators with spectral radius normalization.
        
        Returns:
        - Ehat: normalized symmetric operator
        - Ahat: normalized antisymmetric operator
        - rhoE: spectral radius of E operator
        - rhoA: spectral radius of A operator
        """
        n = len(triples_list)
        ...
        # Normalize generators by spectral radius
        rhoE = self._spectral_radius(E_op)
        rhoA = self._spectral_radius(A_op)
    ...
    def _exact_flow_evolution(self, M0: np.ndarray, Ehat: np.ndarray, Ahat: np.ndarray, rhoE: float, rhoA: float) -> np.ndarray:
        """
        Exact closed-form flow evolution using factorized exponentials.
        
        M(τ) = U_A(τ_A) * M_E(τ_E) * U_A(τ_A)^†
        where:
        M_E(τ_E) = exp(ε*τ_E*Ehat) * M0 * exp(ε*τ_E*Ehat^T)
        U_A(τ_A) = exp(i*ε'*τ_A*Ahat)
        """
        # Calculate normalized flow times
        tauE = self.tau0 / rhoE if rhoE > 0 else 0.0
        tauA = self.tau0 / rhoA if rhoA > 0 else 0.0
        
        # Symmetric "push" by E
        ME = expm(self.epsilon * tauE * Ehat) @ M0 @ expm(self.epsilon * tauE * Ehat.T)
        
        # Antisymmetric "rotation" by A
        U_A = expm(1j * self.epsilon_prime * tauA * Ahat)
    ...
    def _exact_flow_evolution_majorana(self, S0: np.ndarray, Ehat: np.ndarray, Ahat: np.ndarray, rhoE: float, rhoA: float) -> np.ndarray:
        """
        Exact closed-form flow evolution for Majorana neutrinos (symmetric matrix).
        
        S(τ) = U_A(τ_A)^T * S_E(τ_E) * U_A(τ_A)
        where:
        S_E(τ_E) = exp(ε*τ_E*Ehat) * S0 * exp(ε*τ_E*Ehat^T)
        U_A(τ_A) = exp(i*ε'*τ_A*Ahat)
        """
        # Calculate normalized flow times
        tauE = self.tau0 / rhoE if rhoE > 0 else 0.0
        ...
        # Symmetric "push" by E
        SE = expm(self.epsilon * tauE * Ehat) @ S0 @ expm(self.epsilon * tauE * Ehat.T)
        
        # Antisymmetric "rotation" by A (transpose for Majorana)
        U_A = expm(1j * self.epsilon_prime * tauA * Ahat)
        
        # Combined flow for Majorana
    ...
    def _build_sector_with_normalized_flow(self, triples_dict: Dict, sector_key: str, perm_faces: Optional[Tuple[int, int, int]] = None) -> Tuple[List, List, np.ndarray, Dict[str, Any]]:
        """Build sector using normalized flow evolution."""
        ...
        Ehat, Ahat, rhoE, rhoA = self._build_generators(triples_list, gens, sector_key)
        
        # Initialize mass matrix
        M0 = self._initialize_mass_matrix(triples_list, gens)
        
        # Evolve using exact normalized flow
        ...
        normalization_info = {
            "rhoE": float(rhoE),
            "rhoA": float(rhoA),
            "tauE": float(self.tau0 / rhoE if rhoE > 0 else 0.0),
            "tauA": float(self.tau0 / rhoA if rhoA > 0 else 0.0),
            "sector": sector_key
        }
        
        return names, gens, M_evolved, normalization_info
    
    def _build_neutrino_sector_with_normalized_flow(self, triples_dict: Dict, sector_key: str) -> Tuple[List, List, np.ndarray, Dict[str, Any]]:
        """Build neutrino sector using normalized flow evolution for Majorana case."""
        fams = self._sector_family_list(triples_dict, sector_key)
        ...
        M0 = self._initialize_mass_matrix(triples_list, gens)
        S0 = 0.5 * (M0 + M0.T)  # Ensure symmetry
        
        # Evolve using exact normalized flow for Majorana
        ...
        return names, gens, S_evolved, normalization_info
    ...
    def _build_ckm_with_normalized_flow(self) -> Optional[Dict[str, Any]]:
        """Build CKM matrix using normalized flow evolution."""
        names_u, gens_u, Mu, norm_u = self._build_sector_with_normalized_flow(self.triples_q_l, "up", perm_faces=None)
        ...
        best_ckm = None
        for perm in perms:
            names_d, gens_d, Md, norm_d = self._build_sector_with_normalized_flow(self.triples_q_l, "down", perm_faces=perm)
            ...
            if (best_ckm is None) or (score < best_ckm["score"]):
                best_ckm = {
                    ...
                    "norm_up": norm_u, "norm_down": norm_d
                }
        
        return best_ckm
    
    def _build_pmns_with_normalized_flow(self) -> Dict[str, Any]:
        """Build PMNS matrix using normalized flow evolution."""
        # Build charged lepton sector with normalized flow
        names_l, gens_l, Ml, norm_l = self._build_sector_with_normalized_flow(self.triples_q_l, "lepton", perm_faces=None)
        el, Ul = self._diag_hermitian(Ml)
        Ul_pdg = self._reorder_to_pdg(Ul)
        
        # Build neutrino sector with normalized flow
        names_n, gens_n, Sn, norm_n = self._build_neutrino_sector_with_normalized_flow(self.triples_nu, "nu")
    ...
    def tasks(self) -> List[Dict[str, Any]]:
        ...
        return [{
            "task_id": "normalized_flow_implementation",
            "description": "Implement normalized flow with unit-consistent scaling",
            "theoretical_fixes": [
                "theory_consistent_time_nats",
                "generator_spectral_normalization",
                "exact_closed_form_flow",
                "unit_consistent_hierarchy_scaling"
            ]
        }]
    
    @timing_decorator
    def run_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        """Run the normalized flow implementation task."""
        task_id = task["task_id"]
        
        if task_id == "normalized_flow_implementation":
            try:
                # Build CKM with normalized flow
                ckm_result = self._build_ckm_with_normalized_flow()
                ...
                # Build PMNS with normalized flow
                pmns_result = self._build_pmns_with_normalized_flow()
                ...
                # Document generator normalization
                generator_normalization = {
                    "up_sector": ckm_result["norm_up"],
                    "down_sector": ckm_result["norm_down"],
                    "lepton_sector": pmns_result["norm_l"],
                    "neutrino_sector": pmns_result["norm_n"],
                    "normalization_method": "spectral_radius_2norm"
                }
                
                # Create result object
                result = FlowNormalizedResult(
                    ckm_matrix=V,
                    pmns_matrix=U,
                ...
                return {
                    ...
                    "flow_analysis": {
                        "flow_mechanism": "Exact closed-form flow with generator normalization",
                        "time_consistency": f"tau0 = ln(2) * L = {self.tau0:.4f} nats",
                        "hierarchy_source": "Unit-consistent scaling with spectral radius normalization",
    ...
    def summarize(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Summarize the normalized flow results."""
        ...
        generator_normalization = result_data["generator_normalization"]
        ...
        return {
            ...
            "theoretical_approach": "Normalized flow evolution with unit-consistent scaling and exact closed-form evolution"
        }
