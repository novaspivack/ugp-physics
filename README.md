...
The **ugp-lean** companion library provides machine-checked proofs for the UGP framework in Lean 4 (Mathlib). Current status: **308 modules, zero `sorry`, 75 named axioms** (physics structural axioms with documented closure obligations). Most structural results have axiom closure exactly `{propext, Classical.choice, Quot.sound}` — the standard Mathlib signature.
...
- **Kink mass M^Q = 321.32 ± 15.6 MeV** — MS-bar dimensional regularization result; numerically confirmed
...
See [ugp-lean](https://github.com/novaspivack/ugp-lean) for the full theorem inventory. The latest work added 231 net-new theorems.
...
To compile all papers: `bash compile_all_papers.sh --skip-mfrr` (50 papers, ~120 s).
