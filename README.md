...
The **ugp-lean** companion library provides machine-checked proofs for the UGP framework in Lean 4 (Mathlib). Current status: **117 modules, zero `sorry`, zero custom axioms** (standard Mathlib axiom signature).
...
The **srrg-lean** library formalizes the Self-Referential Renormalization Group (SRRG). Key result: **EW VEV = 246.16 GeV** (−0.024% from v_PDG = 246.22 GeV), derived structurally via PSC entropy + S³ Goldstone manifold (grade [A−]; 37 modules, zero sorry in core).
