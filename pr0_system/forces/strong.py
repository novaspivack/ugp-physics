...
class BootstrapPR0:
    ...
    def __init__(self, L_x, L_y):
        ...
        # Standardized history buffer for API compatibility
        self.psi_history = deque(maxlen=20)
        self._annealer = annealing_controller(total_steps=40000.0, min_temperature=0.05)
        self._best_tracker = BestTracker(
            goal="max",
            best_metric=self.best_fitness,
            params={
    ...
    def _meta_learn(self):
        ...
        if self._best_tracker.consider(
            ...
            self.best_fitness = self._best_tracker.best_metric
            self.best_gamma_base = self._best_tracker.restore("gamma_base")
            self.best_gamma_scale = self._best_tracker.restore("gamma_scale")
            print(f"      ✅ NEW BEST: fitness={fitness:.4f}")
        
        # SIMULATED ANNEALING: exploration decreases over time
        temperature = self._annealer.temperature(self.timestep)  # 1.0 → min_temperature
        bounds_base = (0.001, 0.1)
        bounds_scale = (0.1, 2.0)
        
        # Adaptation strategy with temperature-scaled exploration
        if fitness > 0.6:  # Good! Small exploration
            magnitude = 0.1  # ±10% early, ±0.5% late
            self.gamma_base = self._annealer.perturb(self.gamma_base, temperature, magnitude, bounds_base)
            self.gamma_scale = self._annealer.perturb(self.gamma_scale, temperature, magnitude, bounds_scale)
        elif fitness > 0.3:  # OK, explore more
            magnitude = 0.3  # ±30% early, ±1.5% late
            self.gamma_base = self._annealer.perturb(self.gamma_base, temperature, magnitude, bounds_base)
            self.gamma_scale = self._annealer.perturb(self.gamma_scale, temperature, magnitude, bounds_scale)
        else:  # Bad, revert to best + explore
            magnitude = 0.2
            self.gamma_base = self._annealer.revert_to_best(self.best_gamma_base, temperature, magnitude, bounds_base)
            self.gamma_scale = self._annealer.revert_to_best(self.best_gamma_scale, temperature, magnitude, bounds_scale)
    
    def set_soliton(self, x0, y0, amplitude, width, velocity_x=0, charge=None, sign=None):
        """Initialize soliton.
        
        Canonical API uses `charge` (±1). `sign` is accepted for backward compatibility.
        """
        if charge is None and sign is not None:
            charge = sign
        if charge is None:
            charge = +1
