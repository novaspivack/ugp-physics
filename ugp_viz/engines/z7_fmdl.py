...
# Canonical P41 SM generation orbit states on Z_7^5.
# GEN1 is the Garden-of-Eden (GoE) first generation; GEN2 and GEN3 are the
# images of GEN1 and GEN2 under one f_MDL step on the 5-cell periodic ring.
# These values are the authoritative P41 canonical values and are verified
# correct against `fmdl_step5(GEN1)` / `fmdl_step5(GEN2)` respectively.
GEN1 = (1, 5, 2, 2, 1)
GEN2 = (2, 5, 2, 0, 2)   # canonical P41 value: fmdl_step5(GEN1)
GEN3 = (5, 6, 5, 3, 5)   # canonical P41 value: fmdl_step5(GEN2)
VACUUM = (0, 0, 0, 0, 0)
...
class Z7FMDL(SimEngine):
    ...
    spatial_dim = 1
    default_params = {
        "L": 5,          # canonical 5-cell SM orbit ring; periodic BC gives
                         # exact GEN1→GEN2→GEN3→VACUUM orbit. Larger L requires
                         # tiled injection (repeat GEN1 across all L cells) to
                         # maintain correct periodic boundary conditions.
        "dimension": 1,
        "Nx": 16,
        "Ny": 16,
        "Nz": 16,
