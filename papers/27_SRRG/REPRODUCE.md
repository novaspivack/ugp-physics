...
## 083C additions (2026-06-02): SRRG-corrected Higgs quartic

New scripts graduated from research-sandbox:

| Script | Produces |
|---|---|
| `papers/27_SRRG/scripts/higgs_srrg_correction_verification.py` | λ → m_H = 125.2499 GeV; SRRG correction formula verification |
| `papers/27_SRRG/scripts/higgs_ch_ngen_cube_identity.py` | 2c_H+1 = 27 = N_gen³ identity verification |

Run with `python3 <script path>` from the repo root.

New Lean certificates (`ugp-lean/UgpLean/MassRelations/HiggsQuartic.lean`, zero sorry):
- `two_cH_plus_one_eq_ngen_cubed` — 2c_H+1 = N_gen³ = 27
- `higgs_quartic_corrected` — λ formula with SRRG IPT correction
- `higgs_quartic_numerical_approx` — m_H = 125.2499 GeV (−0.0003σ PDG 2022)
