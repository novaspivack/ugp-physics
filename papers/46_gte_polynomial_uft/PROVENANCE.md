...
All from ugp-lean (committed 2026-05-28):
...
- **2026-06-02:** FGCI added — Paragraph in §ssec:nc3 connecting c_H=13, G(n)=F(n) coincidence, and the Frobenius-Generation Coincidence Identity. Lean cert: `fgci_unique_at_nc` (`UgpLean.Universality.FrobeniusChain`, zero sorry).
- **2026-06-10:** §PMDL-Λ duality refined — (i) K_self carrier/record split: ln Z[J] factorizes exactly (Gaussianity from K_extra = 0) into a record-independent measure sector (the carrier, Ω_carrier = 3π/14) and the record self-energy (the gravitational ledger); PMDL-Λ duality sharpened to "gravity = record sector, Λ floor = measure sector of one ln Z[J]"; absolute per-mode pricing gate disclosed. (ii) D_res two-object statement: the irremovable residual is the realized halting-weighted record ledger (limit-computable, not computable); the published 0.6899 re-graded as its census-capacity evaluation (upper bracket); variational firewall noted. Script graduated: `scripts/pmdl_carrier_gaussian_split.py` + results JSON (all certificates re-verified from the new location). Editorial pass: m_W unified to the 1-loop value 80.37 GeV (g = 0.653, v_PSC = 246.16 GeV; 0.003% from PDG 2024); n_s status harmonized with P44 (Lean-certified incl. definitional Z₂-EFT discharge; physical EFE-bridge interpretation open); Lean-inventory table simplified to theorem/module/role columns.
