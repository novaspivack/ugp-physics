...
physical structures: spatial cellular-automaton dynamics achieving Rule~110
Turing universality (Class~4 complexity), conservation of Standard Model
gauge vertices via $\ZS$ winding arithmetic, gravitational coupling via the
Poisson equation $\nabla^2\Phi = G_{\rm eff}\,p(w_x, w_y, w_z)$, quantum
entanglement with negativity $\mathcal{N} = 0.24$, and Bell nonlocality with
CHSH parameter $S = 2.4459$ ($86.5\%$ of the Tsirelson bound $2\sqrt{2}$),
with local hidden-variable models rigorously excluded.
The additional specification cost for each role beyond the first is zero:
$K_{\rm extra} = 0$ in every case.
The $\tau_c$ shared clock satisfies all Page--Wootters prerequisites, yielding a
first-principles derivation of the Born rule
$P(k|\tau) = |\langle k|U(\tau)|\psi_0\rangle|^2$ ($\CatAD$). The 3D Poisson
equation sourced by $p(w_x,w_y,w_z)$ gives the exact Newtonian potential
$\varphi(b) = G_{\rm eff}M\,\mathrm{erf}(b/\sqrt{2}\sigma_{\rm AL})/(4\pi b)$,
recovering $F \propto b^{-2}$ in the classical limit $b\gg\sigma_{\rm AL}$ with
computable corrections $O(\sigma_{\rm AL}/b)^2$ ($\CatAD$).

From the same 19-bit description, applied to the three-tape GTE architecture
in which three cellular automata share a common causal period $\tau_c$
(detailed in the companion paper~\cite{SpivackThreeTapeCMCA}), we derive:
(i) color confinement as a Minimum Description Length (MDL) theorem, with
description-length gap $\Delta K = \log_2(N_c^2) = \log_2 9 \approx 3.17$
bits forbidding free colored quarks; (ii) the number of colors $N_c = 3$ from
the three-tape architecture with no independent assumption; (iii) baryon
number $B = \frac{1}{3}\sum_j \chi_q(w_j)$ as a topological charge of the
$\ZS$ field, with $B = \tfrac{1}{3}$ per quark derived from $N_{\rm tapes} = 3$;
(iv) the SM fermion/boson split identified algebraically with the non-primitive
roots of $\ZSstar$; and (v) lepton-$W$ universality as a consequence of shared
$\ZS$ winding sectors.

The Physical Minimum Description Length (PMDL) variational principle $\delta S_{\rm PMDL}/\delta\Phi = 0$ yields
gravitational attraction as local MDL minimization, while the global MDL
residual $D_{\rm res} > 0$---established from undecidability of the
computational halting problem in the Perfect Self-Containment (PSC)
framework---gives the cosmological constant. The zero-parameter prediction
$\OmegaL = (\ln 2 / 3\pi)\,L_{\rm PSC} = 0.6899$ is within $0.70\sigma$ of
the Planck 2018 value. The vacuum $w = 0$ is proved to be the unique fixed
point of $p(x,x,x) \equiv x \pmod{7}$, providing a first-principles
derivation of vacuum stability. The generating functional
$Z[J] = \int\mathcal{D}\phi\,\exp(-\tfrac{1}{2}\phi(-\Delta)\phi -
p(w_x,w_y,w_z)\phi + J\phi)$ unifies all massless bosonic propagators as
$(-\Delta)^{-1} = 1/(4\pi r)$, derived---not assumed---from the three-tape
causal-graph Laplacian. All foundational results are machine-certified in
...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item PMDL-$\Lambda$ duality: gravity = local PMDL minimization;
        $\Lambda$ = global PMDL residual.
  \item $\OmegaL = (\ln 2/3\pi)\log_2(2000/3) = 0.6899$ ($0.70\sigma$
        from Planck 2018; zero free parameters).
  \item W$^\pm$ as angular mode: $\Delta w = 3$, $m_W = 80.0$ GeV
        ($0.45\%$ from PDG); SU(2)$_L$ gauging from PMDL
...
\input{../common/figures/fig_p46_p_vs_Vphi.tex}
...
\[
  K_{\rm total} = K_{\rm CMCA} + K_{\rm local} + D_{\rm res},
\]
where $K_{\rm CMCA} = 19$ bits is fixed (the specification of $p$),
$K_{\rm local}$ encodes the local field configuration, and $D_{\rm res} > 0$ is
the irreducible MDL residual (Section~\ref{sec:zj}). The local part takes the form
\[
...
\[
  ...
  \underbrace{D_{\rm res}}_{\text{drives }\Lambda}.
\]
...
The residual $D_{\rm res} > 0$ is established by the PSC self-referential
undecidability theorem ($\CatAL$; Lean: \lean{no\_final\_self\_theory}, zero sorry):
no algorithm can decide the halting problem for all PSC configurations, so
$K_{\rm total}$ cannot be globally minimized. The irremovable residual is
$D_{\rm res} = \ln 2 \cdot \log_2(2000/3)$ bits (from the L2 epoch count), giving
\[
  \rho_\Lambda = D_{\rm res}\cdot M_{\rm Pl}^2 \quad\Rightarrow\quad
  \OmegaL = \frac{\ln 2}{3\pi}\log_2\!\left(\frac{2000}{3}\right) = 0.6899.
\]
This is within $0.70\sigma$ of the Planck 2018 value $0.685\pm0.007$, with zero
free parameters.
...
particle-sector loop corrections and RG flow remain open (G27, long-term).
(\textit{Script:} \texttt{papers/35\_gte\_unification/scripts/phimdl\_propagators\_vertices.py}.)

\paragraph{Exact kink S-matrix: all-loop resummed answer (G9 kink sector, $\CatAD$).}
...
This \textbf{closes G9 in the kink/topological sector at CatAD}.

\paragraph{Kink-sector generating functional: all-loop (G27, $\CatAD$).}
...
kink/topological sector without any explicit loop resummation.  This \textbf{closes G27 in the
kink sector at CatAD}: the form-factor bootstrap subsumes all perturbative and
non-perturbative corrections to $Z[J]$ for the topological sector.
...
This particle-sector amplitude is tree-level CatAD; one-loop corrections
are a long-term open sub-problem (G27 particle sector; kink sector $\CatAD$, above).
...
\]
within $0.008\%$ of the Particle Data Group (PDG) value $m_W = 80.377\;\text{GeV}$,
...
CMB tilt $n_s$ (partial)
  & $1 - \ln 2/(2\pi^2) = 0.9649$
  & $0.9649\pm 0.0042$ (Planck)
  & $0.004\sigma$ (2 of 4 steps certified)\\
\bottomrule
\end{longtable}
\end{center}
...
\begin{itemize}[itemsep=2pt]
\item $\OmegaL$: any Planck-precision measurement giving $\OmegaL < 0.667$ or
      $\OmegaL > 0.713$ at $>3\sigma$ would falsify the PMDL residual formula.
...
\item $m_W$: a measured value deviating from $80.0$~GeV by more than $1\%$
      after 2-loop corrections would require re-examination of the SRRG VEV.
\end{itemize}
...
The prediction $n_s = 1 - \ln 2/(2\pi^2) = 0.96488$ (within $0.004\sigma$ of
Planck 2018) has two of four derivation steps Lean-certified:
\begin{itemize}[itemsep=2pt]
\item $\ln 2$ from \lean{rule110\_center1\_is\_nand} (certified).
\item $2\pi^2 = \mathrm{Vol}(S^3)$ from \lean{three\_tape\_causal\_diamond\_t4}
      (certified).
\item The Z$_2$-EFT coupling to the gravitational effective action via Wald
      entropy (open question OQ-QG-1-Z$_2$-EFT).
\end{itemize}
...
Establish the Z$_2$-EFT coupling that connects the CMCA binary gate entropy
$\ln 2$ to the gravitational effective action via the Wald entropy formula.
This is the third step in the CMB tilt derivation and the Level~3 completion
...
constant $\OmegaL = 0.6899$ ($0.70\sigma$ from Planck 2018); the W boson mass
$m_W = 80.0$ GeV; the golden ratio as the SRRG contraction eigenvalue; color
...
\noindent\textbf{Open:}
String tension at Level~2; Lorentzian extension; CMB tilt third step (Z$_2$-EFT).
...
{\small
\begin{longtable}{P{5.0cm}P{4.5cm}P{1.8cm}P{2.5cm}}
\toprule
\textbf{Theorem name} & \textbf{Module} & \textbf{Commit} & \textbf{Physical role}\\
\midrule
\endhead
\lean{gte\_polynomial\_five\_roles\_k\_extra\_zero}
...
\midrule
$\OmegaL$ & $0.6899$ & $0.70\sigma$ from Planck
...
$m_W$ (1-loop) & $80.37$ GeV & $0.008\%$ from PDG
