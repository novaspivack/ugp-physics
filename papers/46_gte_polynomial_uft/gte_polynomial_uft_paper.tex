...
\begin{itemize}[noitemsep,topsep=2pt]
  ...
  \item \textbf{[Gravity/EM Split]} Gravity $= p(w,w,w)$ (degree 3);
        ...
        (\lean{gravity\_em\_degree\_split}).
...
MDL residual $D_{\rm res} > 0$ (Section~\ref{ssec:pmdl_lambda}). Writing
$\mathbf{x}=(x,y,z)$, each tape $j\in\{x,y,z\}$ is an intrinsically
\emph{one-dimensional} array $w_j:\{0,\ldots,L-1\}\to\mathbb{Z}_7$; the notation
$w_j(\mathbf{x})$ denotes $w_j$ evaluated at $\mathbf{x}$'s own $j$-coordinate
only ($w_x(\mathbf{x}):=w_x(x)$, etc.), not an independent function of all three
coordinates --- the reading consistent with the three-tape holographic
state-space count $7^{3L}$ of \S\ref{ssec:3d_universal}
(\lean{three\_tape\_holographic\_L7}, P45~\cite{SpivackThreeTapeCMCA}). The local
part takes the form
\[
...
($\CatAL$; Lean: \lean{baryon\_color\_z3\allowbreak\_orbit\_neutral}, zero sorry.)
...
($\CatAL$; Lean: \lean{gravity\_em\_degree\_split}, zero sorry.)
...
zero axioms; \lean{cmca\_z2\_sublayer\allowbreak\_spectral\_tilt} and related;
...
\lean{baryon\_color\_z3\allowbreak\_orbit\_neutral}
  & \texttt{SU3GluonCount.lean}
