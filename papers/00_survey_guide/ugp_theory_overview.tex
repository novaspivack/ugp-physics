...
% ====================================================================
\subsection{The Substrate: MDL Minimality and the Unique Selection of $\Phi_{\rm MDL}$}
...
\midrule
CMCA lattice (Planck resolution) & ${\sim}10^{-33}$~cm ($\ell_{\rm Pl}$) & ${\sim}10^{19}$~GeV \\
...
\end{tabular}
\end{center}
The CMCA operates at Planck-scale lattice resolution ($a \sim \ell_{\rm Pl} \approx 10^{-33}$~cm), but the kinks that emerge from it are \emph{macroscopic} relative to this scale.  The BPS kink width is $\sim 1/m_\varphi \sim 10^{-13}$~cm (hadronic scale), twenty orders of magnitude larger than the Planck length.  Kinks are therefore not sub-Planckian objects; they are Compton-wavelength-scale collective excitations of the Planck-scale substrate, precisely as phonons are macroscopic excitations of an atomic lattice.  This is the physical content of the no-CA-replica theorem: a finite-resolution CA at any $M$ cannot exactly reproduce the smooth Compton-scale kink profile because the kink extends over $\sim M^{-1}$ lattice sites in the continuum limit.  The Algebraic Lifting Theorem bridges the two scales without requiring a simulation of the Compton scale from first principles.
...
description are the same object.  Second, Turing universality is a structural lower
bound on $\Phi_{\rm MDL}$ (not exact, since no finite CA equals it): the
$\mathrm{GF}(7)$ polynomial representation forces Turing completeness via NAND,
so the algebraic structure certifying $\Phi_{\rm MDL}$ is itself computationally
universal.  Third, the geodesic computation principle (\S\ref{subsec:overview-gravity})
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.25em]
  \item Nine charged-fermion masses at 0.293\% RMS with zero active parameters at
...
\]
This sits $0.24\sigma$ from the PDG measurement.  The $Z$-boson mass $M_Z = 91.914$
...
% ====================================================================
\subsection{QCD and the Strong Force from $F_{21}$}
...
% ====================================================================
\subsection{Computational Universality: A Cook-Independent Proof}
...
(2004)~\cite{Cook2004}.  The UGP Physics programme derives universality by an independent
algebraic route that does not rely on Cook's CTS encoding
construction~\cite{SpivackCookTheorem,SpivackGF7Universality}.

The key result is an exact polynomial representation of the Rule~110 update rule
over the Galois field $\mathrm{GF}(7) = \mathbb{Z}/7\mathbb{Z}$:
\[
...
1913), Turing universality follows immediately and unconditionally from the
polynomial representation alone --- Cook's theorem is a corollary, not a foundation.
The polynomial and NAND identity are Lean-certified with zero sorry
(Lean: \lean{rule110\_z7\_poly\_rep}, \lean{rule110\_center1\_is\_nand}).
Turing universality follows via \lean{phimdl\_turing\_universal}
($\mathbf{A}_{\rm Lean}$ conditional; zero sorry; one named axiom:
\lean{z7\_boolean\_completeness\_implies\_turing\_universal})~\cite{SpivackGF7Universality}.
...
% ====================================================================
\subsection{Quantum Mechanics from the $\Phi_{\rm MDL}$ Field}
...
selection).  This resolves the measurement problem without many-worlds, pilot waves,
...
% ====================================================================
\subsection{Gravity from the $\Phi_{\rm MDL}$ Field}
...
remain quantum-degenerate to all loop orders.  Four zero-sorry Lean theorems
...
($\mathbf{A}_{\rm Lean}$; P47/P43).  Quantum vacuum degeneracy is therefore
a structural theorem, not an assumption.
...
preservation~\cite{SpivackTE24BH}.
...
problem.  The \textbf{GTE Holographic Encoding Theorem} proves that five descriptions
--- Lagrangian, Reed--Solomon $[5,3,3]_7$ algebraic code, holographic/RT, MDL-information-theoretic,
and quantum error correction --- are mutually equivalent, with all twenty directed
implications closed.  The MDL cosmological initial state requires only $\log_2 3
...
\]
matching Planck~2018 at $0.004\sigma$ when the EFE-bridge step is established
...
% ====================================================================
\subsection{The Complete $\Phi_{\rm MDL}$ Framework}
...
$\mathbb{Z}_7$ anomaly-freedom ($\mathbf{A}_{\rm Lean}$; see \S\ref{subsec:overview-gravity}),
...
\midrule
$\sin^2\theta_W$ & $3/13 = 0.23077$ & $0.23122(3)$ & $0.24\sigma$ & $\mathbf{A}_{\rm Lean}$ \\
...
$n_s$ (CMB tilt) & $1 - \ln(2)/(2\pi^2) = 0.96488$ & $0.9649 \pm 0.0042$ (Planck~2018) & $0.004\sigma$ & $\mathbf{A/D}$ (cond.) \\
...
$\Omega_\Lambda$ (PSC route, P47) & $(\ln 2/3\pi)\log_2(2000/3) = 0.6899$;\newline $2000/3 = D^2 N_{\rm fam}^{N_{\rm gen}}/N_{\rm gen} = 4^2{\cdot}5^3/3$ & $0.6889 \pm 0.0056$ (Planck~2018) & $+0.18\sigma$ & CatAD \\
$\Omega_\Lambda$ (holographic, P47) & $3\pi/14 = 0.6732$ & $0.6889$ (Planck~2018) & bracket & CatAD \\
$\Omega_\Lambda$ PSC-adjudication bracket & $[3\pi/14,\; 0.6899]$; $N_{\rm gen}{=}3$ unique integer satisfying bracket & $0.6889$ (Planck~2018) & Planck within bracket & CatAD \\
...
\begin{itemize}[leftmargin=1.5em,itemsep=0.25em]
  ...
  \item \textbf{Quantum cosmological constant.}
    ...
    What remains open is a complete non-renormalization proof showing loop corrections
    to $\Lambda$ are \emph{exactly} zero (not merely suppressed) to all loop orders ---
    i.e.\ a GTE analog of the cosmological non-renormalization theorem (CatD).
...
$3\pi/14 = 0.6732$, bracketing Planck~2018 within the PSC-adjudication band $[3\pi/14,\;0.6899]$ at CatAD), the CMB spectral index ($n_s = 0.96488$, $0.004\sigma$),
the CKM CP phase ($\delta_{CP} = 68.51^\circ$, $0.017\%$ from PDG), and the neutrino
mass sum ($\Sigma m_\nu = 59.4$~meV).  No free parameters are
introduced at any stage; the only true external input is one energy scale ($G_F$,
which fixes $v_H = (\sqrt{2}\,G_F)^{-1/2}$).  The tau-lepton Yukawa $y_\tau = 1/98$
and the structural Koide relation ($\theta = 2/9$ from $N_c = 3$) then derive all
three charged-lepton masses from $v_H$ without any fermion mass anchor.

The ugp-lean library currently comprises 281 Lean~4 modules.  Every Category-A physics
theorem in the library has axiom closure exactly $\{\texttt{propext},\,
\texttt{Classical.choice},\, \texttt{Quot.sound}\}$ --- the standard Mathlib
signature.  No UGP-specific axioms appear in any Category-A physics theorem.
...
The 47 papers of the UGP Physics programme (P01--P47), the ugp-lean Lean~4 library, and the
