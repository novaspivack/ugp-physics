"""
GTE hadronic vacuum polarization (HVP) dispersion estimate.

Order-of-magnitude estimate of a_μ^HVP from GTE hadronic spectrum (P39), not a
precision dispersive calculation. Uses:
  1. GTE masses: π, ρ, ω, φ, K*, K (from P39)
  2. VMD single-ρ estimate (formula with standard 1/3 colour factor)
  3. Simplified multi-resonance + ππ continuum dispersion integral

References:
  - Hagiwara et al., Rev. Mod. Phys. 83, 1547 (2011) — HVP kernel
  - P39 gte_qcd_structure_paper.tex tab:zeropdg
  - GTE pion mass from P39: m_π^GTE = 136.485 MeV; Δa_μ = 2.49×10⁻⁹
...
GTE_SPECTRUM = {
    "pi0/charged_avg": {
        "gte_MeV": 136.485,
        "pdg_MeV": 134.98,
        "source": "P39 GOR NLO inversion",
    },
    "rho770": {
        "gte_MeV": 771.785,  # m_pi^GTE + K_hyp(635.3); GTE pion anchor
        "pdg_MeV": 775.26,
        "gte_lo_MeV": 775.3,
        "source": "P39: mV = mPS + K_hyp; GTE pion replaces 140 MeV",
    },
    "omega782": {
        "gte_MeV": 779.185,  # rho_GTE + 7.4 MeV isospin
        "pdg_MeV": 782.65,
        "gte_lo_MeV": 775.3,
        "source": "P39 LO + isospin correction 7.4 MeV",
    },
    "phi1020": {
        "gte_MeV": 990.995,
        "pdg_MeV": 1019.461,
        "source": "P39 F_21 Berry hyperfine",
    },
    "Kstar892": {
        "gte_MeV": 935.220,
        "pdg_MeV": 891.67,
        "source": "P39 (+4.7% vs PDG)",
    },
    "K_pm": {
        "gte_MeV": 510.5,
        "pdg_MeV": 493.677,
...
# ---------------------------------------------------------------------------
results = {
    "computation": "GTE HVP dispersion estimate",
    "assumptions": [
...
print("=" * 72)
print("GTE HVP dispersion estimate")
print("=" * 72)
print()
