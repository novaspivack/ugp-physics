...
## HVP/Δα_had scripts (graduated 2026-06-03)

Three scripts certifying the GTE hadronic vacuum polarization analysis (P35 §HVP/Δα_had section):

```bash
cd papers/35_gte_unification/scripts
python3 gte_ope_hvp_correction.py        # GTE OPE d=4 condensate contribution to Δα_had
python3 gen_hvp_dispersion_cert.py       # HVP Lean 4 norm_num bound generator (VMD + continuum)
python3 gte_hvp_dispersion_estimate.py  # GTE first-principles HVP estimate via VMD dispersion
```

| Script | Computes | Key result |
|--------|----------|------------|
| `gte_ope_hvp_correction.py` | SVZ OPE d=4 condensate correction to Δα_had using ⟨ψ̄ψ⟩_GTE = −M_kink³; also pQCD u,d,s [2–3 GeV] | OPE NP correction: +0.27% of Δα_had (CatB negative finding — insufficient to close 26.9% gap); pQCD u,d,s [2–3 GeV]: +4.72% |
| `gen_hvp_dispersion_cert.py` | Generates Lean 4 norm_num rational bounds for Δ(1/α)_had via VMD (ρ, ω, φ) + continuum (c, b, pQCD u,d,s) using KSRF convention | Lean preview written to `/tmp/HVPDispersionBounds_preview.lean`; ρ+ω+c+b+pQCD uds coverage: ~73.1% of Δα_had |
| `gte_hvp_dispersion_estimate.py` | Order-of-magnitude estimate of a_μ^HVP from GTE hadronic spectrum (P39 masses) via VMD + ππ continuum dispersion integral | VMD coverage: ~59.0% from ρ+ω+φ sector; total GTE-accessible: ~73.1% |

Artifacts: co-located JSON in `scripts/`. Runtime: <2 min each (timeouts enforced).

Dependencies: Python 3.9+, scipy (gte_ope_hvp_correction.py only).
