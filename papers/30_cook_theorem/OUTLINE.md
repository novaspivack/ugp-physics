...
**Status:** Draft aligned with partial Lean certification. `cook_operational_stage3_tm_microstep_readback` (renamed and rescoped from `rule110_turing_universal_from_cook`) is proved, contingent on five named Cook bridge axioms; revise to unconditional wording when those five axioms close. Full Rule 110 Turing universality additionally requires a general TM$\to$CTS compiler, tracked separately.
...
- `cook_bridge_axioms_open` — what global closure requires; proved satisfied by `cook_bridge_open_predicate_satisfied` (renamed from `cook_bridge_axioms_closed`)
- Top target: `cook_operational_stage3_tm_microstep_readback` (PROVED, contingent on 5 named Cook bridge axioms; renamed and rescoped from `rule110_turing_universal_from_cook`)
...
- `cook_operational_stage3_tm_microstep_readback` (proved; full unconditional Turing universality still requires the TM→CTS compiler for arbitrary machines)
