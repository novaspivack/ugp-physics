...
- **2026-06-02 (083C quality audit — commit bba2d17e):** Added Yukawa overlap exponent from DPP tape counting: α=N_c−1=2 (CatAD: `yukawa_overlap_exponent_catad`); leptogenesis suppression bundle (`leptogenesis_kink_overlap_catad`); sech integrals `integral_sech_cubed`=π/2 and `integral_sech`=π (CatAL, Mathlib FTC). Module: `Gravity/YukawaOverlapExponent.lean`, `Substrate/PhiMDLFluctuationSpectrum.lean`.
- **2026-07-20:** Eq. (pmdl-action) and the "Step A (Local)" source-density formula (§Gravitational Coupling) now state explicitly that each tape `w_j` is a one-dimensional array evaluated only at its own coordinate (`w_x(\mathbf{x}):=w_x(x)`, `w_y(\mathbf{x}):=w_y(y)`, `w_z(\mathbf{x}):=w_z(z)`), consistent with the three-tape holographic state-space count `7^{3L}` (\lean{three_tape_state_card}, CatAL, §Holographic structure).
