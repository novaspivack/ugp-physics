...
**Use the Python scripts for:** large-lattice numerical experiments, gravity pipeline, figure generation, full reproducibility in any Python 3.10+ environment.

**Use the .wl for:** independent verification in Mathematica or Wolfram Engine; single-bit inner clock gating (simpler cross-check than the P41 M=7 version).

### Script roles

| Script | Role |
|--------|------|
| `three_tape_cmca.py` | **Core**: three-tape CMCA simulation (full lattice, CA-native gravity, soliton) |
| `verification_suite.py` | Complete 9-claim verification suite matching ThreeTapeCMCA.wl |
| `run_all_verifications.py` | One-command reproducibility of all P45 numerical results |
| `ThreeTapeCMCA.wl` | Independent Wolfram cross-check of all 9 claims; single-bit inner clock |
...
| `gravity_force_law_continuum_limit.py` | Newtonian F ~ b⁻² continuum limit (CatAD) |
| `bps_instanton_action_derivation.py` | S_{3-tape} = π instanton action |
| `bell_layer_reconciliation.py` | L1/L2 Bell layer distinction (G4) |

### Related code in other papers

| File | Paper | Description |
|------|-------|-------------|
| `papers/41_three_layer_chiral_minkowski_ca/scripts/cmca_full_reproducibility_wolfram_version.wl` | P41 | Single-tape CMCA with M=7 inner clock mini-tape (majority-vote gating); more detailed AFCA architecture |
| `papers/41_three_layer_chiral_minkowski_ca/scripts/cmca_full_reproducibility.py` | P41 | Python single-tape reproducibility suite (9 P41 claims) |
| `papers/49_gte_polynomial_wolfram/scripts/three_tape_wolframmodel_v2.wl` | P49 | Orbit-level DPP causal graph; no cell-level dynamics or SR verification |
...
| **BPS instanton action** | **`bps_instanton_action_derivation.py`** | **S_{3-tape} = π (Routes A, B, algebraic all agree); S₁ = π/3; ε_FN = exp(−π/3) = 0.35092** |
...
## BPS instanton action (§5.1 paragraph, eq. S_3tape)

```bash
cd papers/45_three_tape_cmca/scripts
python3 bps_instanton_action_derivation.py
# Expected output:
#   S_{3-tape} Route A (M_R1 formula):    3.14159265
#   S_{3-tape} Route B (tau × Z7 × BPS): 3.14159265
#   S_{3-tape} algebraic (|Z_7| cancels): 3.14159265
#   Routes agree: True
#   S_1 = pi/N_c = pi/3 = 1.04719755
#   eps_FN = exp(-pi/N_c) = 0.35091981
```

Artifact: `scripts/bps_instanton_action_results.json`

The |Z_7| orbit count cancels algebraically; S_{3-tape} = π depends only on N_c = N_gen = 3.
