...
$\Lambda_{\mathrm{GTE}} \approx 2\,\mathrm{GeV}$ (the GTE kink--antikink creation threshold in physical units).
...
Without this theorem, the certified algebraic structure at Planck-cell scale would lack a formal connection to electrons and quarks; with it, every beable-level result is licensed as a physical prediction.
...
The distinction is physically significant: using $\hbar c \approx 197\,\mathrm{MeV{\cdot}fm}$ and the GTE lattice calibration ($\mathrm{sim\_to\_fm} = 0.100\,\mathrm{fm/sim}$), the proton Compton wavelength $\lambda_C^p = \hbar c/(m_p c^2) \approx 197\,\mathrm{MeV{\cdot}fm}/938\,\mathrm{MeV} \approx 0.21\,\mathrm{fm}$ corresponds to roughly $2$ simulation cells at $M=7$, scaling to order $10^{13}$--$10^{15}$ cells per proton Compton wavelength at lattice resolutions fine enough for individual-cell dynamics to be physically meaningful---placing direct CA construction of a single hadron far beyond any realizable computation.
The existence proof circumvents this barrier entirely: it proceeds from the winding algebra alone, without constructing any explicit CA configuration.
...
($t_a \leq t_b$), with zero custom axioms (\lean{SpatiallyExtendedLifting.lean}).
...
$m_\pi$ (GOR inv.) & 136.49 MeV & 134.98 MeV & computational \\
...
The GTE framework is an effective field theory below a matching scale
\begin{equation}
\Lambda_{\mathrm{GTE}} = N_7 \cdot m_{\rm kink}^{\rm phys} \approx 2.01^{+0.24}_{-0.44}\,\mathrm{GeV}
\qquad \text{(PROVISIONAL)},
\label{eq:lambda_gte}
\end{equation}
calibrated from the BPS kink mass in physical units.
...
\begin{table}[ht]
\centering
\caption{Prediction classification by energy scale relative to $\Lambda_{\mathrm{GTE}} \approx 2\,\mathrm{GeV}$.}
