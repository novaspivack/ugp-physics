...
# (using ∫ sech^2(ax) e^{ikx} dx = (pi k/a^2) / sinh(pi k/(2a)) with a = m):
# T_00(k) = (8m/49) * (pi k/(2m)) / sinh(pi k/(2m))  = (8m/49) * (pi u/2) / sinh(pi u/2)
# where u = k/m
...
print(f"  T_00_hat(k) = (8m/49) * (pi k/(2m)) / sinh(pi k/(2m))")
...
for k_over_m in [0.001, 0.1, 0.5, 1.0, 2.0, 5.0]:
    ...
    if u < 1e-6:
        ...
        ratio = (math.pi * u / 2.0) / math.sinh(math.pi * u / 2.0)
    k_phys = k_over_m * m
...
results = {
    ...
    "Fourier_transform": {
        "formula": "T00_hat(k) = (8m/49) * (pi k/(2m)) / sinh(pi k/(2m))",
