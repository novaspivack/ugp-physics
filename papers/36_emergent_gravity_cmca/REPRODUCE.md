...
## Reproducing Result 10: UV Spectral Dimension of 3+1D CMCA Causal Graph

**Script:** `scripts/spectral_dimension_uv_cmca.py`

```bash
cd papers/36_emergent_gravity_cmca/scripts/
python3 spectral_dimension_uv_cmca.py
```

**Expected output (primary: full eigendecomposition, L=8, T=4, 2048 nodes):**
- d_s(UV, σ≈1) ≈ 2.0 (CDT UV prediction)
- d_s(IR, σ≈5) ≈ 4.0 (CDT IR prediction)

**Key parameters:**
- Method: full Laplacian eigendecomposition (small graph, all eigenvalues) + stochastic trace estimation (large graph verification)
- Wall-clock timeout: 270 s

**Interpretation:**
The 3+1D CMCA causal graph exhibits CDT-consistent spectral dimension running:
d_s ≈ 2 in the UV (short diffusion time) and d_s ≈ 4 in the IR (long diffusion time).
This matches the causal dynamical triangulations prediction for a quantum spacetime
that is effectively 2-dimensional at small scales, confirming the GTE causal graph
carries the expected quantum-gravitational dimensional flow.
...
- Vacuum spatial Gromov--Hausdorff convergence (§6) is machine-certified in Lean 4
  (`vacuum_cmca_gh_converges_to_flat_space`, `ugp-lean/UgpLean/Spacetime/VacuumGHConvergence.lean`,
  zero sorry). The full Lorentzian continuum limit (matter-sector and Lorentzian GH
  convergence, OQ-QG-1b/1c) has no reproducing script; it remains open.
...
| `spectral_dimension_uv_cmca.py` | `scripts/` ✅ |
