...
SC-B & Dual-path mass comparison (UCL~2.3 both arms; theoretical arm: derived $\mathrm{renorm}_K$+URC, $\sigma\approx 0.29\%$) plus coefficient-target table vs.\ Elegant Kernel ($\|\Delta\mathbf{k}\|_\infty\approx 7\times 10^{-3}$; main text Appendix on verifier modes) & \nolinkurl{dual_path_comparison.json} & \texttt{923366a2\ldots} \\
...
\multicolumn{4}{l}{\textit{EW VEV structural derivation (srrg-lean; grade [A$_{\rm Lean}$], zero open axioms --- \texttt{psc\_ew\_entropy\_maximization} is a proved theorem)}} \\[2pt]
SC-EBF25 & $v_{\rm PSC} = 246.16$~GeV structural EW VEV derivation via PSC entropy framework; null-discipline saturation 0.35\% over 288 formula candidates & \nolinkurl{higgs_vev_self_consistent.json} & --- \\
SC-EBF26 & Level~1 $m_H = 124.971$~GeV ($-2.08\sigma$) using self-consistent EW VEV $v_{\rm self} = 246.267$~GeV; same two-loop $g_2$ running as $m_W$ closure & \nolinkurl{higgs_mass_level1_closure.json} & --- \\[6pt]

\multicolumn{4}{l}{\textit{Referee-closure null-discipline bundles (\S4, \S5, \S7.2.1 of main paper)}} \\[2pt]
SC-RC1-URC & URC bounded-uniqueness enumeration: 1.6M depth-$\le 3$ expressions over UGP-structural atoms; UGP-claimed closures outside top-50; 30 feature-randomisation nulls each finding a better best residual $\Rightarrow$ A/D upheld (Open Problem (iii)) & \nolinkurl{comp_p01_RC1_urc_bounded_enumeration.json} & \texttt{b4339642\ldots} \\
...
$v_{\rm PSC}$ (structural EW VEV) & $246.16$~GeV & $-0.024\%$ from $v_{\rm PDG} = 246.22$~GeV & \textbf{Structural derivation} (grade [A$_{\rm Lean}$]; PSC entropy framework; zero open axioms --- \texttt{psc\_ew\_entropy\_maximization} is a proved theorem, zero sorry in srrg-lean; SC-EBF25) \\
$m_H$ Level~1 & $124.971$~GeV & $-2.08\sigma$ from PDG $125.20 \pm 0.11$~GeV & \textbf{Level~1 result} (Grade~C; uses self-consistent $v_{\rm self} = 246.267$~GeV from same two-loop $g_2$ running as $m_W$ closure; 9.1$\sigma$ bare result is a separate open problem and is not superseded; SC-EBF26) \\
...
The following Lean~4 theorems underpin the Category~A structural claims of the main paper.  All reside in the \texttt{ugp-lean} library~\cite{ugp-lean} with universal axiom signature $[\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}]$ (standard Mathlib baseline) and zero \texttt{sorry} in all tactic proofs.\footnote{The library additionally contains two analytic number theory lemmas (\texttt{dickman\_equidistribution\_in\_APs} and \texttt{crt\_equidistribution\_within\_regime}) declared as \texttt{axiom}s citing Tenenbaum III.6; these do not appear in the axiom closure of any physics theorem in this table.  Running \texttt{\#print axioms} on any theorem below returns exactly the standard Mathlib signature.}  The library comprises 118 modules; see the companion formalization paper~\cite{SpivackUGPFormalization} for the full theorem inventory; this table is scoped to the core set underpinning this paper's claims.  The full theorem inventory is catalogued in the companion formalization paper~\cite{SpivackUGPFormalization}.
...
\multicolumn{3}{l}{\textit{Neutrino mass-squared ratio arithmetic bridge (\texttt{MassRelations.NeutrinoMassRatio}; ugp-lean; zero sorry)}} \\[2pt]
\nolinkurl{fn_texture_gives_seesaw_exponent} & \nolinkurl{MassRelations.NeutrinoMassRatio} & FN texture $(q_1,q_2) = (3,2)$ gives $q_1 + q_2/N_c^2 = 3 + 2/9 = 29/9 = \nu_{\rm seesaw}$ \\
\nolinkurl{seesaw_ratio_independent_of_MR} & \nolinkurl{MassRelations.NeutrinoMassRatio} & Mass-squared ratio $R$ is independent of common prefactor $C = E_D^2/M_R$ (algebraic) \\
\nolinkurl{neutrino_mass_ratio_coarse_bound} & \nolinkurl{MassRelations.NeutrinoMassRatio} & $0.029 < R < 0.030$ where $R = (11^{58/9} - 5^{58/9})/(19^{58/9} - 5^{58/9})$; zero sorry \\
\nolinkurl{neutrino_mass_ratio_tight_bound} & \nolinkurl{MassRelations.NeutrinoMassRatio} & $|R - 0.02936| < 0.0001$; proved via unit-width integer bounds on $b^{58/9}$, zero sorry \\
\nolinkurl{neutrino_mass_ratio_within_1pct_of_nufit} & \nolinkurl{MassRelations.NeutrinoMassRatio} & $|R - 0.02951| < 0.01 \times 0.02951$; within 1\% of NuFIT~6.0 central value; zero sorry \\[4pt]
...
\multicolumn{3}{l}{\textit{EW VEV structural derivation (\texttt{SrrgLean.VEVProof} --- srrg-lean; grade [A$_{\rm Lean}$], zero open axioms --- \texttt{psc\_ew\_entropy\_maximization} is a proved theorem)}} \\[2pt]
\nolinkurl{goldstone_volume_correction_per_generation} & \nolinkurl{SrrgLean.VEVProof.GoldstoneEntropyCorrection} & $\varphi^{1/N_{\rm gen}}$ Goldstone volume correction per generation \\
\nolinkurl{psc_entropy_after_contraction} & \nolinkurl{SrrgLean.VEVProof.PSCEntropyDuality} & PSC Entropy-Contraction Duality: entropy after contraction determined by PSC extremum \\
\nolinkurl{ew_vacuum_manifold_uniqueness} & \nolinkurl{SrrgLean.VEVProof.EWGoldstoneManifold} & $S^3$ Goldstone manifold uniqueness: 3 massless bosons, $\mathrm{Vol}(S^3) = 2\pi^2$ \\
\nolinkurl{srrg_physical_fp_implies_ew_vacuum_manifold} & \nolinkurl{SrrgLean.VEVProof.EWVacuumBridge} & Bridge: PhysicalSubspace $\to$ $S^3$ manifold (zero sorry, under the stated PhysicalSubspace hypotheses) \\
\nolinkurl{srrg_vev_no_go} & \nolinkurl{SrrgLean.FixedPoints.VEVNoGo} & SRRG cannot generate $v$ via dimensional transmutation (no-go theorem) \\[4pt]
...
\nolinkurl{ugp_is_turing_universal} & \nolinkurl{Universality.TuringUniversal} & The UGP substrate is Turing universal via its CRT register file simulating Minsky counter machines (conditional on one named classical axiom) \\[6pt]
