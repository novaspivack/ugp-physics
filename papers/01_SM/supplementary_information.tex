...
\multicolumn{4}{l}{\textit{EW VEV structural derivation (srrg-lean; grade [A$^-$], conditional on \texttt{psc\_ew\_entropy\_maximization})}} \\[2pt]
...
$v_{\rm PSC}$ (structural EW VEV) & $246.16$~GeV & $-0.024\%$ from $v_{\rm PDG} = 246.22$~GeV & \textbf{Structural derivation} (grade [A$^-$]; PSC entropy framework; conditional on 1 named open axiom \texttt{psc\_ew\_entropy\_maximization}; zero sorry in srrg-lean; SC-EBF25) \\
...
\multicolumn{3}{l}{\textit{EW VEV structural derivation (\texttt{SrrgLean.VEVProof} --- srrg-lean; grade [A$^-$], conditional on \texttt{psc\_ew\_entropy\_maximization})}} \\[2pt]
...
\nolinkurl{srrg_physical_fp_implies_ew_vacuum_manifold} & \nolinkurl{SrrgLean.VEVProof.EWVacuumBridge} & Bridge: PhysicalSubspace $\to$ $S^3$ manifold (conditional on \texttt{psc\_ew\_entropy\_maximization}) \\
