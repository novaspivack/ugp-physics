...
Full mechanism numerical verification for a rank-structured neutrino mass ansatz.

Proposed synthesis from the decomposition phase (`comp_p01_EBF_21` landscape):
...
where:
  ...
  δ = 7                      (mirror offset; Lean-certified)
  4N_c² − δ = 29             (coincides with exponent numerator)
  29/9 = N_c + θ_Koide       (canonical phase identity)
  θ_Koide = 2/9              (Koide phase; Lean-certified)
...
M_GUT_best_eV  = 2.17e25                # prior absolute-scale calibration point
...
print("=" * 72)
print("COMP-P01-EBF-22 — full mechanism verification")
print("=" * 72)
print(f"""
Proposed mechanism:
...
print(f"""
  PREDICTIONS (at M_GUT = 2.17×10^16 GeV, calibrated absolute-scale reference):
...
print(f"""
  ...
  Key structural identity (Lean target):
    ...
    (both rank-3 QCD bookkeeping constants)
