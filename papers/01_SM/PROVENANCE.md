...
python3 UGP_GTE_SM_Verifier.py \
  --preset-fullstack --n 10 --full-derivation
```
...
The frozen artifact bundle in `papers/01_SM/canonical_run/` was produced by verified UGP_GTE_SM_Verifier runs. The UCL coefficient comparison bundle was **refreshed 2026-06-11** via `comp_p01_ucl_coeff_audit.py` (run record `Verifier_reports/ucl_coeff_audit_20260611-054600/`), incorporating the `m_W` PDG 2024 target alignment (80369.2 MeV) and documentation-flag cleanup; all per-particle masses and both primary σ values are unchanged from the 2026-05-30 freeze (which aligned `calculate_theoretical_coefficients()` with `k_gen = φ cos(π/10)`, Lean `thm_ucl2_fully_unconditional`).

### Key Verified Outputs

| Quantity | Canonical Value | Source |
|----------|----------------|--------|
| Primary σ GoF (empirical) | **≈ 2.95 × 10⁻³ %** (2026-06-11 audit) | `dual_path_comparison.json` |
| Primary σ GoF (theoretical, dual-path) | **≈ 0.295 %** (2026-06-11 audit) | `dual_path_comparison.json` |
| Fully theoretical UCL only (no empirical coeffs) | **≈ 1.11 %** primary σ (2026-06-11) | `fully_theoretical_results.json` |
| `k_gen` (Elegant Kernel) | **1.5388417685876268** | `theoretical_coefficients.json` |
...
| `reference_lock.json` | Frozen σ, W-ρ, masses, hashes (`--verify-reference`) |
| `CANONICAL_HASHES.md` | SHA-256 table for 2026-06-11 verifier bundle |
...
| `theoretical_coefficients.json` | Elegant Kernel vector (`k_gen = φ cos(π/10)`) |
| `fully_theoretical_results.json` | End-to-end run with `THEORETICAL_COEFF_VECTOR` only |
| `fully_theoretical_grand_synthesis.md` | Report for fully theoretical path |
...
| `comp_p01_HMC_higgs_mass_closure.json` | `comp_p01_HMC_higgs_mass_closure.py` | Self-consistent VEV closure: g₂(M_W) = 0.65278; v_self = 246.267 GeV; m_H_self = 124.971 GeV; σ = −2.08σ (Grade C, M_W anchor). Tension reduced from 9.1σ (bare) via two-loop g₂ running. |
...
### 2026-06-11 — Theoretical-path benchmark re-baselined to the canonical artifact

**What changed:** `tab:theoretical_primary` and all dependent text now report the dual-path theoretical arm of the committed canonical artifact `canonical_run/dual_path_comparison.json` (reproduced byte-for-byte by `comp_p01_ucl_coeff_audit.py` on the shipped verifier): per-particle masses 0.510990 / 105.667 / 1775.22 / 2.16097 / 4.67129 / 93.4154 / 1275.53 / 4179.49 / 171159 MeV, primary σ **0.295%** (RMS over the nine charged fermions + W-ρ, PDG 2022-era targets).

**Why:** The previously printed table (0.513 / 105.9 / 1771 / … / 172116 MeV, "0.293% RMS", evenly distributed ±0.2–0.5% residuals) originated in a pre-repository theoretical-path emission using the earlier numeric `g3 = 6/7 + 1/2048` engine constant plus a URC application that was never archived; those per-particle rows are not reproduced by any artifact in the canonical bundle, and their own row-wise RMS (0.35%) did not match the printed 0.293% caption. The 0.293% headline itself traced to a superseded `dual_path_comparison.json` (predating the structural `s_eng_3 = 5/3 − φ/2` substitution in the verifier); the artifact regenerated after that substitution gives 0.295282%, which is what the live pipeline reproduces today. The paper now quotes only values traceable to the committed artifact.

**Also fixed:** the PDG 2024 recompute note now uses the dual-path predictions themselves (0.261% with the PDG 2024 top average 172.57 GeV — an improvement, since the theoretical top mass 171.16 GeV lies below both targets); the previously quoted 0.366% had been computed from a different prediction pipeline (the Koide/TT/VV cyclotomic cascade) against the unarchived table rows and is withdrawn. The standalone "Extended GoF 5.33% across 25 observables" sentence (same unarchived origin) is also withdrawn. The worked electron example now shows the actual dual-path intermediates (C_f^(theo) = 1.11448, m_e^(theo) = 0.510990 MeV). Stale `--full-derivation 1` invocations corrected to `--full-derivation` (boolean flag) in the paper, SI, REPRODUCE.md, this file, and the verifier docs. SI Table S1 hash prefixes refreshed (`dual_path_comparison.json` → `923366a2…`; `comp_p01_RC1_urc_bounded_enumeration.json` → `b4339642…`).

---

### 2026-05-15 — Higgs mass self-consistent VEV: tension reduced from 9.1σ to −2.08σ

**New artifact:** `comp_p01_HMC_higgs_mass_closure.py/.json` — Higgs mass partial closure using self-consistent VEV via two-loop g₂ running.

**Key result:** g₂(M_W) = 0.65278 → v_self = 246.267 GeV → m_H_self = 124.971 GeV → **σ = −2.08σ** (Grade C; M_W external anchor).  Bare-coupling baseline: m_H = 124.2 GeV, ~9.1σ.  The residual −2.08σ tension arises entirely from λ_H = φ/(4π) vs. the SM value (−0.40%).  Structural derivation of v without external EW-scale input remains open (OP(iv)).

**Changes to `standard_model_from_ugp.tex`:**
- Abstract item (7): updated to include self-consistent result (−2.08σ) alongside baseline 9.1σ.
- Status table (Higgs row): updated to two-tier accounting (bare 9.1σ / self-consistent −2.08σ, Grade C / structural v derivation open).
- §subsec:higgs VEV paragraph: added self-consistent VEV formula (Eq. `\eqref{eq:vev_self}`).
- §subsec:higgs mass result: added m_H_self = 124.971 GeV, −2.08σ (Eq. `\eqref{eq:higgs_mass_self}`).
- OP(iv) status paragraph: partial closure status and open programme description updated.
- Category A/D overview paragraph (§4): updated to show self-consistent result.
- Section summary box and table footnote updated.

**Changes to `papers/00_survey_guide/ugp_survey_readers_guide.tex`:**
- Abstract summary, paper table (P01 row), key predictions table: all updated to show self-consistent result.

**Changes to `papers/27_SRRG/srrg_paper.tex`:**
- Appendix B (app:higgs): added note connecting SRRG EW stability result to the self-consistent VEV, identifying the −0.4% λ_H offset as the shared structural source of both residuals.
...
- **2026-05-15 (EPIC_051 sandbox graduation + structural v derivation):**
  - Graduated 10 files from `research_sandbox/HMC_L2_vev_derivation/` to `canonical_run/`:
    `null_discipline_vev_formula.py/.json`, `norfleet_connection.py`,
    `two_loop_srrg_correction.py/.json`, `gap2_genius_team.json`,
    `genius_team_ngen_eff.json`, `proof_psc_duality_synthesis.md`,
    `proof_psc_duality_adam.py`, `proof_psc_duality_jane.py`.
  - Key result: $v_{\rm PSC} = 246.16$~GeV ($-0.024\%$ from $v_{\rm PDG}$), grade [A$^-$].
    Null-discipline saturation 0.35% (288 candidates, 1 hit). Structural, not coincidental.
    Formalized in `SrrgLean.VEVProof.*` (zero sorry; conditional on 1 named open axiom: `psc_ew_entropy_maximization`).
  - §subsec:higgs updated: added "Structural derivation of v" paragraph (grade [A−]).
  - Removed incorrect claim "all four structural paths have been exhausted."
  - REPRODUCE.md updated: EW VEV structural derivation section added.
- **Round G1 (2026-05-16):** PSCEntropyDuality axioms `psc_entropy_contraction_duality` and `srrg_s3_entropy_increase` discharged as proved theorems (zero sorry) in `SrrgLean.VEVProof.PSCEntropyDuality.lean`. VEVProof axiom count: 2 → 1 remaining (`psc_ew_entropy_maximization`). Grade unchanged: [A−]. §subsec:higgs and Appendix VEVProof description updated.
- **2026-06-02 (083C quality audit — commit bba2d17e):** UCL Tier 1/2/3 master certifications added: `elegant_kernel_full_certification` (9/9), `ucl_fermion_mass_ordering` (CatAL), `ucl_lepton_sector_koide_identity` (Tier 3, CatAL). Higgs quartic upgraded to CatAL: `higgs_quartic_corrected`, m_H = 125.2499 GeV (−0.0003σ PDG 2022). Lean cert inventory updated; overfull fixes applied.

- **2026-06-02:** FGCI added — Frobenius-Generation Coincidence Identity showing N_c=3 uniqueness added to §subsec:nc_chain. Lean certs: `fgci_unique_at_nc`, `frobenius_chain_terminates` (`UgpLean.Universality.FrobeniusChain`, zero sorry).
