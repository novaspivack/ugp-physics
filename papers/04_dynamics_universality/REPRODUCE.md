...
## 9) Lean certifications (ugp-lean)

The GTE update map and GoE hierarchy are Lean-certified in ugp-lean:

```bash
cd /path/to/ugp-lean
lake build UgpLean.Universality.GTECompilation
lake build UgpLean.Universality.GoEHierarchy
lake build UgpLean.Universality.GTEUniqueness
lake build UgpLean.Universality.PSCUniversality
```

Key zero-sorry theorems (all in `UgpLean/Universality/`):
- `gte_update_at_seed` (GTECompilation): T(LeptonSeed) = (9,42,1023) — native_decide
- `gte_odd_step_c_is_1023` (GTECompilation): odd step always outputs c = 1023 for all triples
- `gte_gen1_is_garden_of_eden_direct` (GoEHierarchy): LeptonSeed is a GTE GoE (c = 1023 ≠ 823)
- `fmdl_goe_implies_gte_goe` (GoEHierarchy): fMDL GoE → GTE GoE
- `gte_binary_uniqueness` (GTEUniqueness): Rule 110 is unique among lawful binary CAs
- `psc_implies_computational_universality` (PSCUniversality): PSC → CU

Remaining sorrys (all documented as hard blockers):
- `sigma_gte`, `uwca_eval`, `gte_compilation_theorem` (GTECompilation): require UWCA tile
  language on Triple type (P08 §4.3 bridge infrastructure, not yet in Lean)
- `IsLawfulUWCAProgram`, `UWCABisim`, `gte_uniqueness_up_to_bisimulation` (GTEUniqueness):
  same blocker (proper UWCATileSet type, not `List ℕ`)
