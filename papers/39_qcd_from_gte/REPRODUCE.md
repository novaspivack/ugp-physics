...
lake build UgpLean.Universality.LambdaGTEThreshold
```
...
| `leptonic_sector_heaviest_gen3` | Spacetime/OrbitMassHierarchy.lean §7 | Leptonic sector gen₃ heaviest (SCC prerequisite) |
| `mphi_equals_tau_mass_scc` | Spacetime/OrbitMassHierarchy.lean §7 | m_φ = m_τ = 1776.86 MeV via Self-Consistency Condition |
| `mkink_from_scc` | Spacetime/OrbitMassHierarchy.lean §7 | M_kink = (8/49) m_τ = 290.10 MeV |
| `fpi_from_scc` | Spacetime/OrbitMassHierarchy.lean §7 | f_π = M_kink/π = 92.34 MeV (+0.30% vs PDG) |
| `weak_isospin_identification` | Universality/WeakIsospin.lean | SU(2)_L doublet structure from Z₇ W_B arithmetic (CatAL) |
| `lambda_gte_threshold_identity` | Universality/LambdaGTEThreshold.lean | Λ_GTE arithmetic skeleton: (8/7)m_τ identity; multiplier minimality; coset charges = all nonzero residues; 7 = 11 − 4 slope continuity |
...
Matching scale: Λ_GTE = 7·M_kink ≈ 2.0 GeV, derived (tree (8/7)m_τ =
2030.70 ± 0.14 MeV; pole 7M^Q = 1.970 ± 0.146 GeV; envelope 1.96 ± 0.15 GeV) —
see the Λ_GTE threshold derivation section below.
Systematic: f_quant ≈ 0.63 (classical–quantum correction, documented open issue).

---

### Λ_GTE first-principles threshold derivation

Scripts (in `papers/39_qcd_from_gte/scripts/`):

| Script | Produces |
|---|---|
| `lambda_gte_band_threshold_derivation.py` | Λ_GTE = 7·M_kink per kink-mass reading: tree (8/7)m_τ = 2030.70 ± 0.14 MeV (band 0.007%); pole 7M^Q = 1.970 ± 0.146 GeV (M^Q = 281 ± 21 MeV, P42 GJQW interface dim-reg); envelope 1.96 ± 0.15 GeV; tree/pole agreement +0.41σ; multiplier neighbour nulls; f_π PCAC cross-check |
| `lambda_gte_band_route_battery.py` | Running-intersection route closed negative (drift 8.7×10⁻¹⁸ — b₀ = 7 on both sides makes the boundary RG-invisible at one loop); coset-window coherence; self-breakdown candidates + wrong-target/neighbour nulls |
| `lambda_gte_band_sigma_verdict.py` | e² = 7/2 ↔ PDG σ-verdict at the derived Λ_GTE readings (pole +1.89σ / tree +3.06σ at c = 0); μ* = 2.357 GeV; α_s(M_Z) anchor-chain sensitivity (tree +0.17%, pole −0.33%) |

Run from the scripts directory:

```bash
cd papers/39_qcd_from_gte/scripts
python3 lambda_gte_band_threshold_derivation.py
python3 lambda_gte_band_route_battery.py
python3 lambda_gte_band_sigma_verdict.py
```

Each writes a `*_results.json` artifact alongside the script.
The pole-mass input is the corrected one-loop domain-wall pole mass
M^Q = 281 ± 21 MeV (P42, `kink_pole_mass_*` scripts in
`papers/42_phimdl_field/scripts/`).

Lean certification of the arithmetic core:
...
## 083C additions (2026-06-02): Muon g−2 analysis

New scripts graduated from research-sandbox:

| Script | Produces |
|---|---|
| `papers/39_qcd_from_gte/scripts/muon_g2_gte.py` | a_μ^GTE = 7.47×10⁻¹¹ (Φ_MDL scalar loop); honest null — 3% of data-driven anomaly (CatA) |
| `papers/39_qcd_from_gte/scripts/muon_g2_deeper_analysis.py` | Five-direction investigation: SRRG Yukawa, soliton form factor, orbit-ratio formula, BMW 2020 MDL preference |

Run with `python3 <script path>` from the repo root.

*REPRODUCE.md — P39 — 2026-05-29*

## 083D additions: Light quark mass cascade

New script graduated from research-sandbox:

| Script | Produces |
|---|---|
| `papers/39_qcd_from_gte/scripts/gte_light_quark_mass_stage1.py` | Full TT+VV cascade for all five non-top quarks (m_u, m_d, m_s, m_c, m_b) from (m_e, m_μ) alone |

Expected outputs (PDG 2024, CatB; max |σ| = 0.156σ):

| Observable | GTE | PDG 2024 | σ |
|---|---|---|---|
| m_u | 2.157 MeV | 2.16 ± 0.38 MeV | −0.009 |
| m_d | 4.647 MeV | 4.70 ± 0.34 MeV | −0.156 |
| m_s | 92.74 MeV | 93.5 ± 5.85 MeV | −0.129 |
| m_c | 1270.7 MeV | 1270 ± 90 MeV | +0.008 |
| m_b | 4184.3 MeV | 4183 ± 8 MeV | +0.156 |

Run with `python3 papers/39_qcd_from_gte/scripts/gte_light_quark_mass_stage1.py`.
