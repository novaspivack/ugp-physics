...
def sample_paths(KU, KPT, pi, T, npaths, seed):
    rng = default_rng(seed)
    N = len(pi)
...
def run_case(params):
    N, kind, beta, T, job_seed = params
    rng = default_rng(job_seed)
    A = make_adj(N, kind)
    KU, pi = make_KU(N, density=0.6 if kind!="dense" else 1.0, seed=job_seed)
    ...
    ends = sample_paths(KU, KPT, pi, T, npaths=50_000, seed=job_seed+1000)
    dS = F[ends[:,1]] - F[ends[:,0]]
    est = np.mean(np.exp(-dS))
    mean = float(np.mean(dS))
    return [N, kind, beta, T, est, mean]
...
def main():
    ...
    for N in Ns:
        for kind in kinds:
            for beta in betas:
                for T in Ts:
                    jobs.append((N, kind, beta, T, seed))
                    seed += 1
    
    print(f"Running {len(jobs)} cases with multiprocessing...")
    ...
    plt.xlabel("T"); plt.ylabel(r"$\langle e^{-\Delta S_{ref}}\rangle$")
