...
def _ewk_predict_w_mass_mev(sin2_theta_w: float = 0.25934302, rho_factor: float = 1.049) -> float:
    try:
        m_harm = int(os.environ.get('Z2_M_EW', '0'))
    except Exception:
        m_harm = 0
    if m_harm in (1,2,3):
        # Tiny deterministic adjustment tied to Z2 δ (bounded to ±0.002)
        d = _z2_ckm_delta(m_harm)
        eps = (d - (math.acos(1.0/3.0))) / math.pi  # ~[-0.1, +0.1] scaled by π
        sin2_theta_w = float(max(0.20, min(0.26, sin2_theta_w + 0.02*eps)))
