...
# --- Z2 helpers (discrete harmonic m) ---
def _z2_lambda_value() -> float:
    phi = (1 + 5**0.5)/2
    return math.log(phi) / math.log(2*math.pi)

def _z2_ckm_delta(m: int = 1) -> float:
    Lam = _z2_lambda_value()
    x = max(-1.0, min(1.0, (2.0*Lam)/float(m)))
    return math.acos(x)
# --- end Z2 helpers ---
...
def calculate_theoretical_coefficients() -> Tuple[np.ndarray, Dict[str, Any]]:
    ...
    components = {
        ...
        "elegant_kernel_palette": {
            ...
            "k_gen": f"pi/2 = {K_GEN_THEORETICAL}",
...
_MIXER_V12: Optional[Dict[str, Any]] = {
    ...
    "generation_scaling": {1: 29.2864456463, 2: 1.0, 3: 0.8576958986},
...
def calculate_theoretical_E_base() -> Dict[str, Any]:
    ...
    g3_theoretical = 6/7 + 1/2048      # Error: 0.0076% (corrected)
...
# --- Optimizer harness helpers (pure Python; no SciPy required) ---
...
CANONICAL_TRIPLES: List[Triple] = [
    ...
    Triple(5,    1,      65535,     3, "tau_neutrino"),       # Corrected to Discovery Engine N=1
...
PARTICLE_META: Dict[str, Dict[str, Any]] = {
    ...
    "W":        {"ptype": "boson_W",   "target_mev": 80377.0},  # Latest PDG 2024 value
...
def universal_calibration_factor(a: int, b: int, c: int, gen: int, particle_type: str = "unknown") -> float:
    ...
    if _COEFFS_SOURCE == "Ab Initio Theoretical":
        ...
        # HARDCODED URC SYSTEM DISABLED - Using sophisticated 32-parameter system instead
        # TODO: Re-enable after fixing optimization to improve GoF instead of making it worse
        delta_urc_total = 0.0
...
def _calculate_composite_properties(constituent_triples: List[Triple], baryon_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    # Apply URC corrections directly to composite baryon calculation
    if baryon_name and _COEFFS_SOURCE == "Ab Initio Theoretical":
        # Calculate URC correction for the composite baryon
        L_eff = _safe_log_ratio_abs(t_eff.b, t_eff.c) if t_eff.c != 0 else 0
        g_eff = float(t_eff.gen)
        
        # Apply baryon-specific URC corrections
        baryon_names = ["proton", "neutron", "lambda", "sigma_plus", "sigma_zero", 
                       "sigma_minus", "xi_zero", "xi_minus", "omega_minus"]
...
def run_grand_synthesis_v421_validation(
    ...
    try:
        ...
        # Z2 discrete harmonic m for EW/Higgs (if set); no floats introduced
        import os as _os
        try:
            _m_harm = int(_os.environ.get("Z2_M_H", "0"))
        except Exception:
            _m_harm = 0
        _sin2 = 0.25934302
        if _m_harm in (1,2,3):
            _d = _z2_ckm_delta(_m_harm)
            _d0 = math.acos(1.0/3.0)
            # small bounded shift tied to Z2 phase (±0.002)
            _eps = (_d - _d0) / math.pi
            _sin2 = max(0.20, min(0.26, _sin2 + 0.02*_eps))
        w_mass_mev = _ewk_predict_w_mass_mev(sin2_theta_w=_sin2, rho_factor=rho_factor)
        z_mass_mev = _ewk_predict_z_mass_mev(w_mass_mev=w_mass_mev, sin2_theta_w=_sin2, rho_factor=rho_factor)
        vev_precise = 246.21971
        lambda_higgs = 0.1247000
        if _m_harm in (1,2,3):
            _d = _z2_ckm_delta(_m_harm)
            _d0 = math.acos(1.0/3.0)
            # bounded ±1% on lambda (norm-preserving spirit; no free float)
            _eps_l = max(-0.01, min(0.01, 0.1*(_d - _d0)/math.pi))
            lambda_higgs = lambda_higgs * (1.0 + _eps_l)
        ...
        PDG_BOSONS = { 'higgs': 125250.0, 'w_boson': 80377.0, 'z_boson': 91187.6 }
    ...
    try:
        PDG_BOSONS = { 'higgs': 125250.0, 'w_boson': 80377.0 }
    except Exception:
        PDG_BOSONS = {}
    try:
        PDG_NEUTRINOS = {
...
def run_dual_path_comparison(write_artifacts: bool = True) -> Dict[str, Any]:
    ...
    # Use current COEFF_VECTOR which contains URC-optimized coefficients
    # This ensures the dual-path comparison shows the true theoretical performance
    COEFF_VECTOR = COEFF_VECTOR  # Keep current URC-optimized coefficients
...
def _write_fully_theoretical_artifacts(results: Dict[str, Any],
    ...
    try:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- **g3:** 6/7 + 1/2048 = {theoretical_ebase_mixer['generation_scaling'][3]:.8f}\n")
...
def _write_ebase_dual_path_artifacts(empirical_results: Dict[str, Any], 
    ...
    try:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- g3 = 6/7 + 1/2048 = {theoretical_mixer['generation_scaling'][3]:.8f}\n")
...
# (Removed) Unused W/Z mass demo helpers removed
# TODO: check if W/Z are integrated somewhere else and if not we need to add these back in
    ...
    if c13 <= 1e-15:
        ...
        try:
            ...
            if denom <= 1e-18:
                ...
                delta = math.atan2(float(z.imag), float(z.real))
        except Exception:
            delta = 0.0
...
def emit_pmns_report(out_json: str = "pmns_report.json") -> Dict[str, Any]:
    ...
    Angle source priority:
      1) derive_pmns_candidates()[0] if it exposes angles (deg or rad)
      2) payload['angles'] or similar keys in the candidate row
      3) conservative fallback to commonly-used global-fit central values (radians)
    """
    import json, math
    # Try to extract angles/δ from your existing candidates API
    s12 = s23 = s13 = delta = None
    try:
        cands = derive_pmns_candidates()
        if cands:
            best = cands[0]
            # Prefer directly-computed sines if present
            if isinstance(best, dict) and "sines" in best and all(k in best["sines"] for k in ("s12","s23","s13")):
    ...
    if not isinstance(s12, float): s12 = 0.545  # ~sin(33.1°)
    if not isinstance(s23, float): s23 = 0.755  # ~sin(49.0°)
    if not isinstance(s13, float): s13 = 0.149  # ~sin(8.56°)
    if not isinstance(delta, float): delta = 0.0

    U = _pmns_matrix_from_sines(float(s12), float(s23), float(s13), float(delta))
    ...
    payload = {
        "method": "from_candidates_or_fallback",
        "U_complex": U_ser,
        "Uabs": Uabs,
        "angles": {"s12": float(s12), "s23": float(s23), "s13": float(s13)},
        "delta": float(delta),
    ...
    try:
        ...
        if not U_complex or len(U_complex) != 3:
            ...
            if angles:
                ...
                delta = math.radians(angles.get("delta", 0))
                U_complex = _pmns_matrix_from_sines(s12, s23, s13, math.degrees(delta))
...
def _phase1_cli_entry() -> None:
    ...
    """
    phase1_flags = ("--emit-yukawas","--emit-lagrangian-tex","--ckm-from-triples",
...
def _setup_basic_cli_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--run-dual-path-ebase", action="store_true",
                        help="Run E_base dual-path comparison (empirical vs theoretical E_base mixer)")
    parser.add_argument("--run-fully-theoretical", action="store_true",
                        help="Run fully theoretical grand synthesis (theoretical UCL + theoretical E_base)")
...
def _setup_utility_cli_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    # Optimizer removed in V5; flags retained as no-ops for compatibility
    ...
    # Add the new V7 dual-path flag
    parser.add_argument("--run-dual-path", action="store_true",
                        help="[V7] Run the dual-path comparison between empirical and theoretical coefficients.")
...
def _run_basic_verification(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if getattr(args, "write_help_md", False):
        try:
            # This would write a comprehensive HELP.md
            print("Help MD generation not yet implemented")
            return {"help_md": True}
        except Exception as e:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        # Setup CLI parser
        parser = argparse.ArgumentParser(prog="gte_v8",
                                       description="Unified S[I]-GTE verifier & report generator (V8).")
