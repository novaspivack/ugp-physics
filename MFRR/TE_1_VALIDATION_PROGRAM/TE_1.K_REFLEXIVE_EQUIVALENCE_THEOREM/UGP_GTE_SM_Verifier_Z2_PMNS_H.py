...
# --- Z2 phase helpers (CKM/PMNS/EW/H) ---
...
def _ewk_predict_z_mass_mev(w_mass_mev: float, sin2_theta_w: float = 0.25934302, rho_factor: float = 1.049) -> float:
    try:
        m_harm = int(os.environ.get('Z2_M_H', '0'))
    except Exception:
        m_harm = 0
    if m_harm in (1,2,3):
        # Compute a tiny deterministic offset tied to Z2 δ; bounded +-0.001 on sin2θw
        d = _z2_ckm_delta(m_harm)
        eps = (d - (math.acos(1.0/3.0))) / math.pi
        sin2_theta_w = float(max(0.20, min(0.26, sin2_theta_w + 0.01*eps)))
