...
# --- Z2 → CKM delta mapping (added by patch) ---
Z2_M_HARMONIC: int = 1  # default m=1; can be reassigned by the runner
...
def _z2_ckm_delta(m: int = 1) -> float:
    """Discrete SU(2) half-turn mapping for the CKM CP phase.
    Use δ = arccos( (2πΛ)/(mπ) ) = arccos(2Λ/m), with Λ = ln φ / ln (2π).
    This yields δ ≈ 1.022 rad (m=1), 1.306 rad (m=2), 1.395 rad (m=3).
    """
    Lam = _z2_lambda_value()
    x = max(-1.0, min(1.0, (2.0*Lam)/float(m)))
    return math.acos(x)
# --- end Z2 → CKM delta mapping ---
...
def ckm_from_masses_gst_fixed() -> Dict[str, Any]:
    ...
    delta = _z2_ckm_delta(Z2_M_HARMONIC)  # Z2-mapped CKM δ
    ...
    def information_to_mass(
        ...
        try:
            ...
            try:
                ...
                if False and _PHASE_DELTA_G3 is not None and int(generation) == 3:
