...
# --- Z2 Ω-completion helpers (added by patch) ---
def _z2_hypercomplex_g_over_pi(m: int = 1) -> float:
    """Return g(Ω)/π from Z₂ half-turn closure for given harmonic m (m=1 minimal)."""
    phi = (1 + 5**0.5)/2
    Lam = math.log(phi) / math.log(2*math.pi)
    two_pi_L = 2*math.pi*Lam
    g = math.sqrt(max(0.0, (m*math.pi)**2 - (two_pi_L)**2))
    return g / math.pi  # ≈ 0.851927 for m=1

def _z2_phase_gain(m: int = 1) -> float:
    """Norm-preserving real lift (SU(2) → ℝ⁺). Keep magnitude = 1.0; use m for discrete audits."""
    return 1.0
# --- end Z2 helpers ---
    ...
    def information_to_mass(
        ...
        try:
            ...
            try:
                ...
                if False and _PHASE_DELTA_G3 is not None and int(generation) == 3:  # Z2 patch: disable ad-hoc per-sector deltas
