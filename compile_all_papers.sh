...
# All papers use plain bibtex. Each paper is built with three pdflatex passes:
#   Pass 1: pdflatex (builds .aux)
#   Pass 2: bibtex (resolves .bbl from .aux + .bib)
#   Pass 3: pdflatex (incorporates .bbl into PDF)
#   Pass 4: pdflatex (resolves cross-references fully)
#
# Usage:
...
compile_paper() {
  ...
  (
    ...
    # Bibliography — run bibtex if the .aux contains a \bibdata command
    # (handles both local .bib files and central ../bib/ references)
    if grep -q "\\\\bibdata" "${stem}.aux" 2>/dev/null; then
...
compile_paper "$REPO/papers/00_survey_guide" \
  "ugp_survey_readers_guide.tex"                   "P00"
...
compile_paper "$REPO/papers/12_unified_rigidity" \
  "Unified_Rigidity_Theorem.tex"                    "P12"
...
compile_paper "$REPO/papers/27_SRRG" \
  "srrg_paper.tex"                                  "P27"

compile_paper "$REPO/papers/28_computational_universality" \
  "computational_universality_ugp.tex"              "P28"

compile_paper "$REPO/papers/29_dark_sector_braid_atlas" \
  "Dark_Sector_Braid_Atlas_Paper.tex"               "P29"

compile_paper "$REPO/papers/30_cook_theorem" \
  "cook_theorem_paper.tex"                          "P30"

compile_paper "$REPO/papers/31_weinberg_angle" \
  "weinberg_angle_paper.tex"                        "P31"

compile_paper "$REPO/papers/32_ckm_matrix" \
  "ckm_matrix_paper.tex"                            "P32"

compile_paper "$REPO/papers/33_deeper_consequences" \
  "deeper_consequences_paper.tex"                   "P33"

compile_paper "$REPO/papers/34_gte_mobius_substrate" \
  "gte_mobius_substrate_paper.tex"                  "P34"

compile_paper "$REPO/papers/35_gte_unification" \
  "gte_unification_paper.tex"                       "P35"

compile_paper "$REPO/papers/36_emergent_gravity_cmca" \
  "emergent_gravity_paper.tex"                      "P36"

compile_paper "$REPO/papers/37_quantum_mechanics" \
  "quantum_mechanics_paper.tex"                     "P37"

compile_paper "$REPO/papers/38_emergent_gravity_phimdl" \
  "emergent_gravity_gte_phimdl.tex"                 "P38"

compile_paper "$REPO/papers/39_qcd_from_gte" \
  "gte_qcd_structure_paper.tex"                     "P39"

compile_paper "$REPO/papers/40_gf7_polynomial_universality" \
  "gf7_polynomial_universality.tex"                 "P40"

compile_paper "$REPO/papers/41_three_layer_chiral_minkowski_ca" \
  "three_layer_chiral_minkowski_ca_paper.tex"       "P41"

compile_paper "$REPO/papers/42_phimdl_field" \
  "phimdl_field_paper.tex"                          "P42"

compile_paper "$REPO/papers/43_phimdl_completeness" \
  "phimdl_completeness_paper.tex"                   "P43"

compile_paper "$REPO/papers/44_quantum_gravity" \
  "quantum_gravity_completeness.tex"                "P44"

compile_paper "$REPO/papers/45_three_tape_cmca" \
  "three_tape_cmca_paper.tex"                       "P45"

compile_paper "$REPO/papers/46_gte_polynomial_uft" \
  "gte_polynomial_uft_paper.tex"                    "P46"

compile_paper "$REPO/papers/47_gte_cosmology" \
  "gte_cosmology_paper.tex"                         "P47"

compile_paper "$REPO/papers/48_gte_complete_theory" \
  "gte_complete_theory.tex"                         "P48"

compile_paper "$REPO/papers/49_gte_polynomial_wolfram" \
  "gte_polynomial_wolfram.tex"                      "P49"

compile_paper "$REPO/papers/50_spin7_lattice" \
  "spin7_lattice.tex"                               "P50"

compile_paper "$REPO/papers/51_polynomial_transputation" \
  "polynomial_transputation.tex"                    "P51"
